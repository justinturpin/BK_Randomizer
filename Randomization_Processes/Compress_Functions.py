...
from .Common_Functions import get_address_endpoints, leading_zeros
from .Dicts_And_Lists.Misc_Dicts_And_Lists import (
    furnace_fun_questions_pointer_list,
    skip_these_setup_pointer_list,
)
from .Dicts_And_Lists.Setups import (
    asm_setup_ids,
    level_model_ids,
    setup_ids,
    speech_file_ids,
...
class Compressor:
    ...
    def __init__(self, master: "User_GUI_Class") -> None:
        self.master = master
        self.paths = master.paths

    def _extract_unchanged_setup(self, addr: str) -> None:
        """
        For every setup file not randomized, pull it out of the ROM into a compressed file.
        """

        file_bytes = self.paths.rom_path.read_bytes()
        (address1, address2) = get_address_endpoints(file_bytes, addr)
        path = self.paths.final_compressed_path(addr)
    ...
    def _compress_file(self, file_name: str) -> None:
        """
        Compresses the hex file that was extracted from the main ROM file
        """

        # Use python gzip library for now. In the future we may want to use a
        # gzip.exe of a specific version.
        # Or better yet, implement gzip in Python, why not.

        raw_data = self.paths.decompressed_path(file_name).read_bytes()
        compressed_data = zlib.compress(raw_data)
        self.paths.compressed_path(file_name).write_bytes(compressed_data)

    def _post_compress_operations(
        self,
        file_name: str,
        header: str,
        footer: str,
        ...
        with self.paths.compressed_path(file_name).open("r+b") as comp_file:
            ...
            with self.paths.final_compressed_path(file_name).open(
                "w+b"
            ) as new_comp_file:
    ...
    def _insert_into_rom_by_pointer(
        self,
        setup_pointer_start,
        setup_pointer_end,
        additional_skip_these_pointer_list=[],
    ) -> None:
        for index_dec in range(setup_pointer_start, setup_pointer_end + 1, 8):
            with self.paths.rom_path.open("r+b") as bk_rom:
                ...
                if index_hex_str in additional_skip_these_pointer_list:
                    ...
                    path = self.paths.final_compressed_path(index_hex_str)
                    with path.open("r+b") as setup_bin:
                        setup_content = setup_bin.read()
                        # Find The Pointer Start
                        pointer_start = ""
                        for offset in range(4):
                            pointer_start += leading_zeros(
                                mm_bk_rom[index_dec + offset], 2
                            )
                        ...
                        setup_count = 0
                        for index in range(
                            address_start, address_start + len(setup_content)
                        ):
                        ...
                        if index_dec != setup_pointer_end:
                            address_end = (
                                address_start + len(setup_content) - int("0x10CD0", 16)
                            )
    ...
    def _insert_into_rom_by_location(self, section_dict):
        for subsection in section_dict:
            if (subsection == "Rusty_Bucket_Bay") and (
                self.master.buttons_var.get() == 0
            ):
                pass
            elif (subsection == "Gobis_Valley") and (
                self.master.matching_puzzle_var.get() == 0
            ):
                pass
            else:
                address_start = None
                    ...
                    with self.paths.rom_path.open("r+b") as bk_rom:
                        ...
                        with self.paths.final_compressed_path(file_name).open(
                            ...
                            if not address_start:
                                ...
                                self._adjust_bootloader_code(
                                    mm_bk_rom, address_start, subsection
                                )
                            else:
                                address_end = int(subsection_info[0].split(",")[1], 16)
                                self._adjust_bootloader_vars(
                                    mm_bk_rom,
                                    address_start,
                                    address_end,
                                    len(setup_content),
    ...
    def _adjust_bootloader_code(self, mm, code_address, subsection):
        ...
        if lower > 0x7FFF:
            upper += 1
        mm[bootloader_asm_dict[subsection]["ROM_Offset_Upper"]] = int(
            leading_zeros(upper, 4)[:2], 16
        )
        mm[bootloader_asm_dict[subsection]["ROM_Offset_Upper"] + 1] = int(
            leading_zeros(upper, 4)[2:], 16
        )
        mm[bootloader_asm_dict[subsection]["ROM_Offset_Lower"]] = int(
            leading_zeros(lower, 4)[:2], 16
        )
        mm[bootloader_asm_dict[subsection]["ROM_Offset_Lower"] + 1] = int(
            leading_zeros(lower, 4)[2:], 16
        )
    ...
    def _verify_pointer_header(self, setup_pointer_start, setup_pointer_end) -> None:
        with self.paths.rom_path.open("r+b") as bk_rom:
            mm_bk_rom = mmap(bk_rom.fileno(), 0)
            for pointer_index in range(setup_pointer_start, setup_pointer_end + 1, 8):
                address_start = (
                    int(
                        (
                            leading_zeros(mm_bk_rom[pointer_index], 2)
                            + leading_zeros(mm_bk_rom[pointer_index + 1], 2)
                            + leading_zeros(mm_bk_rom[pointer_index + 2], 2)
                            + leading_zeros(mm_bk_rom[pointer_index + 3], 2)
                        ),
                        16,
                    )
                    + 0x10CD0
                )
                if (mm_bk_rom[address_start] != 0x11) or (
                    mm_bk_rom[address_start + 1] != 0x72
                ):
                    print(
                        f"Pointer {hex(pointer_index)}:"
                        f" Header At Address {hex(address_start)} Does Not Start With 11 72"
                    )
                    raise SystemExit(1)

    def _subection_compression_main(
        self, file_name: str, header, footer, padding_text: str = "AA"
    ) -> None:
        # Decomp Size
        ...
        with self.paths.decompressed_path(file_name).open("r+b") as rand_comp_file:
            mm_decomp = mmap(rand_comp_file.fileno(), 0)
            decomp_len = leading_zeros(len(mm_decomp), 8)
        # Compress File
        self._compress_file(file_name)
        self._post_compress_operations(
            file_name, header, footer, decomp_len, padding_text
        )

    def _section_compression_main(
        self,
        section_dict,
        setup_pointer_start=None,
        setup_pointer_end=None,
        additional_skip_these_pointer_list=[],
    ):
        ...
        file_list = os.listdir(self.paths.randomized_rom_dir)
        for index_dec in range(setup_pointer_start, setup_pointer_end + 1, 8):
            index_hex_str = str(hex(index_dec))[2:]
            decompressed_path = self.paths.decompressed_path(index_hex_str)
            final_compressed_path = self.paths.final_compressed_path(index_hex_str)
            # TODO: need to figure out why its doing this check. This is basically saying,
            # if index_hex_str was never compressed before and has no decompressed version.
            if not decompressed_path.exists() and not final_compressed_path.exists():
                self._extract_unchanged_setup(index_hex_str)
        # For every modified file,
        #     Compress it and reformat it to prepare for insertion
        for subsection in section_dict:
            for subsection_info in section_dict[subsection]:
                self._subection_compression_main(
                    file_name=subsection_info[0],
                    header=subsection_info[1],
                    footer=subsection_info[2],
                )
        # Insert the files and verify the pointers are correct
        self._insert_into_rom_by_pointer(
            setup_pointer_start, setup_pointer_end, additional_skip_these_pointer_list
        )
        self._verify_pointer_header(setup_pointer_start, setup_pointer_end)

    def _location_compression_main(self, section_dict) -> None:
        # For every modified file,
        #     Compress it and reformat it to prepare for insertion
        for subsection in section_dict:
            if (subsection == "Rusty_Bucket_Bay") and (
                ...
                self._subection_compression_main(
                    file_name=section_dict[subsection][0][0],
                    header=section_dict[subsection][0][1],
                    footer=section_dict[subsection][0][2],
                    padding_text=None,
                )
                self._subection_compression_main(
                    file_name=section_dict[subsection][1][0],
    ...
    def _main(self) -> None:
        ...
        self._section_compression_main(
            setup_ids, 0x9780, 0x9C40, skip_these_setup_pointer_list
        )  # Setups
        self.master.logger.debug("Intro & Lair & Game Over Cutscenes")
        self._section_compression_main(
            speech_file_ids, 0xD150, 0xD230
        )  # Intro & Lair & Game Over Cutscenes
        ...
        self._section_compression_main(
            speech_file_ids, 0xCF90, 0xCF98
        )  # Bottles Tutorial Confirm & Secret Game Text
        self.master.logger.debug("Bottles Move Texts")
        self._section_compression_main(
            speech_file_ids, 0xAFD0, 0xDA30
        )  # Bottles Move Texts
        if self.master.skip_furnace_fun_var.get() == 1:
            self.master.logger.debug("Lair Speeches & Brentilda Hints")
            self._section_compression_main(
                speech_file_ids, 0xDA88, 0xFFB8, furnace_fun_questions_pointer_list
            )  # Gruntilda Lair Speeches, Brentilda Hints, Furnace Fun & Whatever Is In Between
        else:
            if (
                ...
                self._section_compression_main(
                    speech_file_ids, 0xDBD8, 0xDBE8, furnace_fun_questions_pointer_list
                )
        self.master.logger.debug("BK Models Through Jinjonator")
        self._section_compression_main(
            texture_setup_ids, 0x7900, 0x8920
        )  # BK Model - Jinjonator
        ...
        if self.master.gruntilda_difficulty_var.get() > 0:
            ...
            self._section_compression_main(
                level_model_ids, 0x10678, 0x10678
            )  # Final Battle Area 1
