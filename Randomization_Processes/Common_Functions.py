...
def leading_zeros(num: int | str, num_of_digits: int) -> str:
    """
...
def read_json(json_file_dir: str | Path) -> dict[str, Any]:
    """
...
def dump_json(json_file_dir: str | Path, use_this_dict: dict[str, Any]) -> None:
    """
    Reads contents of a JSON file into a dictionary and returns that dictionary
    """

    with open(json_file_dir, "w+") as jf:
        json.dump(use_this_dict, jf, indent=4)
...
def get_address_endpoints(file_bytes, addr) -> tuple[int, int]:
    """
    Goes to address (found in Banjo's Backpack) and address 8 bytes after to find
    the start and end of a setup file
    """
    byte_list = []
    ...
    address1 = int(
        "0x" + byte_list[0] + byte_list[1] + byte_list[2] + byte_list[3], 16
    ) + int("0x10CD0", 16)
    address2 = int(
        "0x" + byte_list[8] + byte_list[9] + byte_list[10] + byte_list[11], 16
    ) + int("0x10CD0", 16)
