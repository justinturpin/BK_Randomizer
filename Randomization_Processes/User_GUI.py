...
tool_tips_dict = {
    ...
    "STARTING_AREA": {
        "NEW_GAME": "Spawns you at the location upon a new game.",
    },
    "ENEMIES": {
        "FRAME": "NONE:\n"
        + "    Skips the setting.\n"
    ...
    "MAP_CONFIG": {
        "FRAME": "Every checkbox with an (A) means it applies aethetic changes\n"
        + "like model or animation replacements. Every checkbox with (P)\n"
        + "changes the death property of an object (like an enemy).",
    },
    "SOUNDS_MUSIC": {
        "FULL_DESCRIPTION": "SHUFFLE_SOUNDS: Shuffles short sounds, like ones for eggs, notes, and feathers.\n"
...
from Randomization_Processes.Common_Functions import dump_json, leading_zeros, read_json
...
from Randomization_Processes.Progression_GUI import Progression_GUI_Class
...
def Error_GUI(error_msg):
    ...
    frames = [
        tk.PhotoImage(
            master=error_window,
            file=(f"{os.getcwd()}/Pictures/Bottles_Speaking.gif"),
    ...
    error_label = tk.Label(
        error_window, text=error_msg, background="#F3E5AB", font=("LITHOGRAPH-BOLD", 12)
    )
...
class WARNING_GUI:
    ...
    def main(self):
        ...
        self.frames = [
            tk.PhotoImage(
                master=self.warning_window,
                file=(f"{os.getcwd()}/Pictures/Klungo_Speaking.gif"),
                format="gif -index %i" % (i),
            )
...
class User_GUI_Class:
    """
    Creates a GUI where users give the directory of the ROM file, select options
    for the randomization, and optionally provide a seed value
    """

    def __init__(self, BK_RANDO_VERSION: str) -> None:
        """Creates the Tkinter GUI"""
        self.BK_RANDO_VERSION = BK_RANDO_VERSION
        self.cwd = Path(os.getcwd())
        self.paths = BkRandoPaths(self.cwd)
        self.app_window = tk.Tk()
        self.app_window.winfo_toplevel().title(
            f"Banjo-Kazooie Randomizer v{self.BK_RANDO_VERSION}"
        )
        self.padx = 3
        self.pady = 1
        self.white = "#FFFFFF"
        ...
        handler = RotatingFileHandler(
            self.cwd / "Randomizer_Log_File.log", maxBytes=(512 * 1024), backupCount=1
        )
    ...
    class CreateToolTip(object):
        ...
        def __init__(self, widget, master, text="widget info") -> None:
            self.widget = widget
            self.master = master
            self.text = text
    ...
    def _select_rom_file(self):
        ...
        filename = tkinter.filedialog.askopenfilename(
            initialdir=self.cwd,
            title="Select The BK ROM File",
            filetypes=(("Rom Files", "*.z64"), ("all files", "*.*")),
        )
        if not filename:
            return
    ...
    def _random_note_value(self) -> None:
        """
        Randomly selects a note value
        """
    ...
    def _random_puzzle_value(self) -> None:
        """
    ...
    def _random_bk_model_preset(self) -> None:
    ...
    def _random_hex(self, digit_len: int) -> str:
        """
    ...
    def _update_bk_model(self, *args):
        ...
        if bk_model_preset in self.bk_model_json:
            ...
            self.tooth_necklace_var.set(
                self.bk_model_json[bk_model_preset]["Tooth_Necklace"]
            )
            ...
            self.kazooie_primary_var.set(
                self.bk_model_json[bk_model_preset]["Kazooie_Primary"]
            )
            self.kazooie_secondary_var.set(
                self.bk_model_json[bk_model_preset]["Kazooie_Secondary"]
            )
            self.kazooie_wing_primary_var.set(
                self.bk_model_json[bk_model_preset]["Kazooie_Wing_Primary"]
            )
            self.kazooie_wing_secondary_var.set(
                self.bk_model_json[bk_model_preset]["Kazooie_Wing_Secondary"]
            )
            self.backpack_var.set(self.bk_model_json[bk_model_preset]["Backpack"])
            self.wading_boots_var.set(
                self.bk_model_json[bk_model_preset]["Wading_Boots"]
            )
            self.shorts_vertex_var.set(
                self.bk_model_json[bk_model_preset]["Shorts_Vertex"]
            )
            self.shorts_texture_var.set(
                self.bk_model_json[bk_model_preset]["Shorts_Texture"]
            )
            preset_image_path = (
                self.cwd / "Pictures" / "BK_Models" / f"{bk_model_preset}.png"
            )

            if os.path.isfile(preset_image_path):
                self.bk_model_image = tk.PhotoImage(file=preset_image_path)
            else:
                self.bk_model_image = tk.PhotoImage(
                    file=(self.cwd / "Pictures" / "BK_Models" / "Question_Mark.png")
                )
    ...
    def _enemy_option_select(self, *args):
        ...
        if self.enemies_var.get() == "Randomize":
            self.enemy_checklist_frame.grid(
                row=2,
                column=0,
                columnspan=6,
                padx=self.padx,
            ...
            self.non_softlock_enemies_button.grid(
                row=0, column=2, padx=self.padx, pady=self.pady, sticky="e"
            )
            self.clear_enemies_button.grid(
                row=0, column=3, padx=self.padx, pady=self.pady, sticky="e"
            )
        else:
    ...
    def _random_customization(self):
        ...
        if self.hiding_customization:
            ...
            self.random_customization_button.configure(
                text="Random By Seed\nAnd Hide Options"
            )
            for checkbutton_count, checkbutton in enumerate(
                self.customization_checkbuttons
            ):
                checkbutton.grid(
                    row=(checkbutton_count // 4) + 1,
                    column=(checkbutton_count % 4),
                    padx=self.padx,
                    pady=self.pady,
                    sticky="w",
                )
    ...
    def _lock_final_puzzle_value(self, *args):
        ...
        if self.final_puzzle_var.get() == 0:
            ...
            self.remove_floating_jiggies_checkbox.grid(
                row=3, column=3, padx=self.padx, pady=self.pady, sticky="w"
            )
    ...
    def _lock_struct_options(self, *args):
        ...
        if self.struct_var.get() == "No Shuffle":
            ...
            self.struct_note_count_var.set("Produce Extra Notes")
            self.struct_note_count_dropdown.grid_remove()
            self.note_overflow_dropdown.grid(
                row=0,
                column=2,
                columnspan=2,
                padx=self.padx,
            ...
            self.struct_note_count_dropdown.grid(
                row=1,
                column=2,
                columnspan=2,
                padx=self.padx,
    ...
    def _save_bk_colors(self):
        ...
        if new_custom_name == "Default":
            ...
            warning_gui = WARNING_GUI(
                f"Are you sure you want to overwrite this preset?\n{new_custom_name}"
            )
        else:
            warning_gui = WARNING_GUI(
                f"Are you sure you want to save this new preset?\n{new_custom_name}"
            )
        ...
        if confirmation:
            ...
            dump_json(
                self.cwd
                / "Randomization_Processes"
                / "Misc_Manipulation"
                / "Model_Data"
                / "BK_Model_Presets.json",
                self.bk_model_json,
            )
            ...
            self.bk_model_dropdown = ttk.Combobox(
                self.bk_model_frame,
                textvariable=self.bk_model_var,
                foreground=self.black,
                background="#F3E5AB",
                font=(self.font_type, self.small_font_size),
                width=30,
            )
            self.bk_model_dropdown["values"] = self.bk_model_options
            self.bk_model_dropdown["state"] = "readonly"
            self.bk_model_dropdown.grid(
                row=0,
                column=1,
                columnspan=2,
                padx=self.padx,
    ...
    def _delete_bk_colors(self):
        ...
        if self.bk_model_var.get() == "Default":
            ...
            if confirmation:
                ...
                dump_json(
                    self.cwd
                    / "Randomization_Processes/Misc_Manipulation/Model_Data/BK_Model_Presets.json",
                    self.bk_model_json,
                )
                self.bk_model_options = [
                    "Seed Determined Preset",
                    "Seed Determined Colors",
                ]
                self.custom_color_count = 0
                for item in sorted(self.bk_model_json):
                    self.bk_model_options.append(item)
                ...
                self.custom_bk_model_name_var.set(
                    f"Custom Preset {self.custom_color_count}"
                )
    ...
    def _skip_furnace_fun_checkbox_trace(self, *args):
        ...
        if self.skip_furnace_fun_var.get() == 1:
            ...
            self.brentilda_hints_dropdown.grid(
                row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
            )
        else:
    ...
    def _gruntilda_difficulty_trace(self, *args):
        ...
        if self.gruntilda_difficulty_var.get() == 0:
            ...
            self.what_floor_checkbox.grid(
                row=4, column=1, padx=self.padx, pady=self.pady, sticky="w"
            )
            self.grunty_size_var.set(1)
            self.grunty_size_checkbox.grid(
                row=5, column=1, padx=self.padx, pady=self.pady, sticky="w"
            )
            self.monster_house_var.set(1)
            self.monster_house_checkbox.grid(
                row=6, column=1, padx=self.padx, pady=self.pady, sticky="w"
            )
    ...
    def _add_randomizer_settings_to_code(self, add_val, counter_add=1):
        ...
        self.generated_randomizer_settings_code += (
            int(add_val) << self.randomizer_settings_count
        )
    ...
    def _generate_randomizer_settings_code(self):
        ...
        ### General Settings ###
        # Flagged Objects
        self._add_randomizer_settings_to_code(
            ["No Shuffle", "Shuffle (World)", "Shuffle (Game)"].index(
                self.flagged_object_var.get()
            ),
            2,
        )
        self._add_randomizer_settings_to_code(
            self.flagged_object_abnormalities_var.get()
        )
        ...
        self._add_randomizer_settings_to_code(
            [
                "Base Game Costs",
                "World Order Scaled Costs",
                "Free Transformations",
            ].index(self.free_transformations_var.get()),
            2,
        )
        self._add_randomizer_settings_to_code(
            [
                ...
                "Two Health Only",
                "One Health Only",
                "Zero Health (Unbeatable)",
                "Random Health Option",
            ].index(self.max_health_banjo_var.get()),
            3,
        )
        self._add_randomizer_settings_to_code(self.remove_floating_jiggies_var.get())
        # Non-Flagged Objects
        self._add_randomizer_settings_to_code(
            ["No Shuffle", "Shuffle (World)"].index(self.non_flagged_object_var.get())
        )
        self._add_randomizer_settings_to_code(
            ["Default Jinjo Colors", "Random Jinjo Colors"].index(
                self.jinjo_color_var.get()
            )
        )
        self._add_randomizer_settings_to_code(
            self.non_flagged_object_abnormalities_var.get()
        )
        ...
        self._add_randomizer_settings_to_code(
            [
                ...
                "Randomize",
                "All Notes",
            ].index(self.struct_var.get()),
            3,
        )
        self._add_randomizer_settings_to_code(
            ["Allow Save & Quit/Reset", "Possible No Save & Quit/Reset"].index(
                self.note_overflow_var.get()
            )
        )
        self._add_randomizer_settings_to_code(
            ["Produce Extra Notes", "Produce Exactly Enough Notes"].index(
                self.struct_note_count_var.get()
            )
        )
        self._add_randomizer_settings_to_code(
            ["Scaling Note Doors", "Final Note Door Only"].index(
                self.final_note_door_var.get()
            )
        )
        ...
        if self.before_blue_egg_carry_value.get() == "?":
            ...
            self._add_randomizer_settings_to_code(
                self.before_blue_egg_carry_value.get(), 9
            )
        if self.before_blue_egg_carry_value.get() == "?":
            self._add_randomizer_settings_to_code(256, 9)
        else:
            self._add_randomizer_settings_to_code(
                self.after_blue_egg_carry_value.get(), 9
            )
        if self.before_blue_egg_carry_value.get() == "?":
            self._add_randomizer_settings_to_code(256, 9)
        else:
            self._add_randomizer_settings_to_code(
                self.before_red_feather_carry_value.get(), 9
            )
        if self.before_blue_egg_carry_value.get() == "?":
            self._add_randomizer_settings_to_code(256, 9)
        else:
            self._add_randomizer_settings_to_code(
                self.after_red_feather_carry_value.get(), 9
            )
        if self.before_blue_egg_carry_value.get() == "?":
            self._add_randomizer_settings_to_code(256, 9)
        else:
            self._add_randomizer_settings_to_code(
                self.before_gold_feather_carry_value.get(), 9
            )
        if self.before_blue_egg_carry_value.get() == "?":
            self._add_randomizer_settings_to_code(256, 9)
        else:
            self._add_randomizer_settings_to_code(
                self.after_gold_feather_carry_value.get(), 9
            )
        # World Entrances
        self._add_randomizer_settings_to_code(
            ["No Shuffle", "Basic Shuffle", "Bottles Shuffle"].index(
                self.world_entrance_var.get()
            ),
            2,
        )
        self._add_randomizer_settings_to_code(
            [
                "Exit From World You Were Just In",
                "Exit From Entrance You Entered From",
            ].index(self.world_exit_var.get())
        )
        self._add_randomizer_settings_to_code(self.all_starting_moves_var.get())
        # Within World Warps
        self._add_randomizer_settings_to_code(
            ["No Shuffle", "Shuffle By World", "Shuffle By Game"].index(
                self.within_world_warps_var.get()
            ),
            2,
        )
        # Starting World
        ...
        self._add_randomizer_settings_to_code(
            starting_world_options.index(self.new_area_var.get()), 8
        )
        self._add_randomizer_settings_to_code(self.skip_intro_cutscenes_var.get())
        # Enemies
        self._add_randomizer_settings_to_code(
            ["Default Enemies", "Shuffle", "Randomize"].index(self.enemies_var.get()), 2
        )
        self._add_randomizer_settings_to_code(
            [
                "Random Size Setting",
        ...
        self._add_randomizer_settings_to_code(
            [
                "Base Game Brentilda Hints",
                "Vague Brentilda Rando Hints",
                "Detailed Brentilda Rando Hints",
            ].index(self.brentilda_hints_var.get()),
            2,
        )
        ...
        # Treasure Trove Cove
        self._add_randomizer_settings_to_code(
            ["No Scatter", "Low Scatter", "High Scatter"].index(
                self.scattered_structs_var.get()
            ),
            2,
        )
        ...
        # Click Clock Wood
        self._add_randomizer_settings_to_code(
            ["Season", "Within World"].index(self.ccw_var.get())
        )
    ...
    def _get_randomizer_setting(self, bit_count=1, options_list=None):
        ...
        self.applied_randomizer_settings_code = (
            self.applied_randomizer_settings_code >> bit_count
        )
        if options_list:
    ...
    def _apply_randomizer_settings_code(self):
        ...
        try:
            ...
            ### General Settings ###
            # Flagged Objects
            self.flagged_object_var.set(
                self._get_randomizer_setting(
                    bit_count=2,
                    options_list=["No Shuffle", "Shuffle (World)", "Shuffle (Game)"],
                )
            )
            ...
            self.free_transformations_var.set(
                self._get_randomizer_setting(
                    bit_count=2,
                    options_list=[
                        "Base Game Costs",
            ...
            # Non-Flagged Objects
            self.non_flagged_object_var.set(
                self._get_randomizer_setting(
                    options_list=["No Shuffle", "Shuffle (World)"]
                )
            )
            self.jinjo_color_var.set(
                self._get_randomizer_setting(
                    options_list=["Default Jinjo Colors", "Random Jinjo Colors"]
                )
            )
            self.non_flagged_object_abnormalities_var.set(
                self._get_randomizer_setting()
            )
            ...
            self.note_overflow_var.set(
                self._get_randomizer_setting(
                    options_list=[
                        "Allow Save & Quit/Reset",
                        "Possible No Save & Quit/Reset",
                    ]
                )
            )
            self.struct_note_count_var.set(
                self._get_randomizer_setting(
                    options_list=["Produce Extra Notes", "Produce Exactly Enough Notes"]
                )
            )
            self.final_note_door_var.set(
                self._get_randomizer_setting(
                    options_list=["Scaling Note Doors", "Final Note Door Only"]
                )
            )
            ...
            if before_red_feather_carry_value == 256:
                ...
                self.before_red_feather_carry_value.set(
                    str(before_red_feather_carry_value)
                )
            ...
            if after_red_feather_carry_value == 256:
                ...
                self.after_red_feather_carry_value.set(
                    str(after_red_feather_carry_value)
                )
            ...
            if before_gold_feather_carry_value == 256:
                ...
                self.before_gold_feather_carry_value.set(
                    str(before_gold_feather_carry_value)
                )
            ...
            if after_gold_feather_carry_value == 256:
                ...
                self.after_gold_feather_carry_value.set(
                    str(after_gold_feather_carry_value)
                )
            # World Entrances
            self.world_entrance_var.set(
                self._get_randomizer_setting(
                    bit_count=2,
                    options_list=["No Shuffle", "Basic Shuffle", "Bottles Shuffle"],
                )
            )
            self.world_exit_var.set(
                self._get_randomizer_setting(
                    options_list=[
                        "Exit From World You Were Just In",
                        "Exit From Entrance You Entered From",
            ...
            # Within World Warps
            self.within_world_warps_var.set(
                self._get_randomizer_setting(
                    bit_count=2,
                    options_list=["No Shuffle", "Shuffle By World", "Shuffle By Game"],
                )
            )
            # Starting World
            starting_world_options = [option for option in start_level_ids]
            starting_world_options.insert(
                0, "Random Starting Area (Auto Have All Moves)"
            )
            self.new_area_var.set(
                self._get_randomizer_setting(
                    bit_count=8, options_list=starting_world_options
                )
            )
            self.skip_intro_cutscenes_var.set(self._get_randomizer_setting())
            # Enemies
            self.enemies_var.set(
                self._get_randomizer_setting(
                    bit_count=2,
                    options_list=["Default Enemies", "Shuffle", "Randomize"],
                )
            )
            self.enemy_size_var.set(
                self._get_randomizer_setting(
                    bit_count=4,
                    options_list=[
                        "Random Size Setting",
            ...
            ### Aesthetic Settings ###
            for custom_name in sorted(self.map_config_checkbox_dict):
                self.map_config_checkbox_dict[custom_name].set(
                    self._get_randomizer_setting()
                )
            ### World Specific ###
            # Gruntilda's Lair
            self.skip_furnace_fun_var.set(self._get_randomizer_setting())
            self.brentilda_hints_var.set(
                self._get_randomizer_setting(
                    bit_count=2,
                    options_list=[
                        "Base Game Brentilda Hints",
            ...
            # Treasure Trove Cove
            self.scattered_structs_var.set(
                self._get_randomizer_setting(
                    bit_count=2,
                    options_list=["No Scatter", "Low Scatter", "High Scatter"],
                )
            )
            ...
            # Click Clock Wood
            self.ccw_var.set(
                self._get_randomizer_setting(options_list=["Season", "Within World"])
            )
            self.ccw_open_var.set(self._get_randomizer_setting())
        except IndexError:
            Error_GUI(
                f"Error: Something went wrong with applying the settings.\nPlease check your settings code."
            )
    ...
    def _set_recommended_defaults(self):
        ...
        self.tooth_necklace_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Tooth_Necklace"]
        )
        self.banjo_skin_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Banjo_Skin"]
        )
        self.banjo_feet_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Banjo_Feet"]
        )
        self.kazooie_primary_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Kazooie_Primary"]
        )
        self.kazooie_secondary_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Kazooie_Secondary"]
        )
        self.kazooie_wing_primary_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Kazooie_Wing_Primary"]
        )
        self.kazooie_wing_secondary_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Kazooie_Wing_Secondary"]
        )
        self.backpack_var.set(self.bk_model_json[self.bk_model_var.get()]["Backpack"])
        self.wading_boots_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Wading_Boots"]
        )
        self.shorts_vertex_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Shorts_Vertex"]
        )
        self.shorts_texture_var.set(
            self.bk_model_json[self.bk_model_var.get()]["Shorts_Texture"]
        )
        # Models, Animations, Properties
        for custom_name in self.map_config_checkbox_dict:
            self.map_config_checkbox_dict[custom_name].set(0)
    ...
    def _load_configuration(self, button_press=True, random_file=False):
        ...
        if random_file:
            try:
                list_of_files = os.listdir(self.cwd / "Configurations")
                if "Last_Used_Configuration.json" in list_of_files:
                    list_of_files.remove("Last_Used_Configuration.json")
                list_of_configs = [
                    file for file in list_of_files if (file.endswith(".json"))
                ]
                filename = self.cwd / f"Configurations/{choice(list_of_configs)}"
                ...
                Error_GUI(
                    "Error Occurred During Random Configuration.\nLeaving Settings As They Are."
                )
                return
        elif button_press:
            try:
                config_default_dir = self.cwd / "Configurations"
                filename = tkinter.filedialog.askopenfilename(
                    initialdir=config_default_dir,
                ...
                Error_GUI(
                    "JSON File Was Not Found Or Operation Was Canceled.\nLeaving The Settings As They Are."
                )
                return
            except Exception:
                Error_GUI(
            ...
            try:
                ...
                Error_GUI(
                    "Last Used Configuration File Not Found.\nImplementing The Default Settings!"
                )
                return
            except Exception:
                Error_GUI(
        ...
        try:
            ...
            self.flagged_object_var.set("Shuffle (World)")
        try:
            self.flagged_object_abnormalities_var.set(
                json_data["Flagged_Objects_Abnormalities"]
            )
        except KeyError:
        ...
        try:
            self.non_flagged_object_abnormalities_var.set(
                json_data["Non_Flagged_Objects_Abnormalities"]
            )
        except KeyError:
        ...
        try:
            self.before_red_feather_carry_value.set(
                json_data["Before_Cheato_Red_Feathers"]
            )
        except KeyError:
        ...
        try:
            self.after_red_feather_carry_value.set(
                json_data["After_Cheato_Red_Feathers"]
            )
        except KeyError:
        ...
        try:
            self.before_gold_feather_carry_value.set(
                json_data["Before_Cheato_Gold_Feathers"]
            )
        except KeyError:
        ...
        try:
            self.after_gold_feather_carry_value.set(
                json_data["After_Cheato_Gold_Feathers"]
            )
        except KeyError:
        ...
        for enemy_name in self.enemy_checkbox_dict:
            try:
                self.enemy_checkbox_dict[enemy_name].set(
                    json_data[f"Include {enemy_name}"]
                )
            except KeyError:
        ...
        try:
            ...
            self.banjo_fur_var.set(
                self.bk_model_json[self.bk_model_var.get()]["Banjo_Fur"]
            )
        try:
        ...
        try:
            ...
            self.backpack_var.set(
                self.bk_model_json[self.bk_model_var.get()]["Backpack"]
            )
        try:
        ...
        for custom_name in self.map_config_checkbox_dict:
            try:
                self.map_config_checkbox_dict[custom_name].set(
                    json_data[f"Include {custom_name}"]
                )
            except KeyError:
        ...
        for short_sound_type in self.short_sounds_dict:
            for short_sound_name in self.short_sounds_dict[short_sound_type]:
                try:
                    (self.short_sounds_dict[short_sound_type][short_sound_name]).set(
                        json_data[f"Include {short_sound_name}"]
                    )
                except KeyError:
        ...
        for jingle_name in self.jingles_dict:
            try:
                (self.jingles_dict[jingle_name]).set(
                    json_data[f"Include {jingle_name}"]
                )
            except KeyError:
        ...
        for music_type in self.music_dict:
            for music_name in self.music_dict[music_type]:
                try:
                    (self.music_dict[music_type][music_name]).set(
                        json_data[f"Include {music_name}"]
                    )
                except KeyError:
        ...
        if setting_not_found:
            if len(setting_not_found) < 6:
                ...
                Error_GUI(
                    f"Error: {len(setting_not_found)} Settings Weren't Set!\nPlease manually set your settings."
                )
            return
    ...
    def _set_random_settings(self):
        ...
        # Flagged Objects
        self.flagged_object_var.set(
            choice(["No Shuffle", "Shuffle (World)", "Shuffle (Game)"])
        )
        ...
        self.free_transformations_var.set(
            choice(
                ["Base Game Costs", "World Order Scaled Costs", "Free Transformations"]
            )
        )
        ...
        self.jinjo_color_var.set(
            choice(["Default Jinjo Colors", "Random Jinjo Colors"])
        )
        ...
        self.note_overflow_var.set("Allow Save & Quit/Reset")
        self.struct_note_count_var.set(
            choice(["Produce Extra Notes", "Produce Exactly Enough Notes"])
        )
        self.final_note_door_var.set(
            choice(["Scaling Note Doors", "Final Note Door Only"])
        )
        ...
        self.after_blue_egg_carry_value.set(
            randint(int(self.before_blue_egg_carry_value.get()), 255)
        )
        self.before_red_feather_carry_value.set(randint(0, 255))
        self.after_red_feather_carry_value.set(
            randint(int(self.before_red_feather_carry_value.get()), 255)
        )
        self.before_gold_feather_carry_value.set(randint(0, 255))
        self.after_gold_feather_carry_value.set(
            randint(int(self.before_gold_feather_carry_value.get()), 255)
        )
        # World Entrances
        self.world_entrance_var.set(
            choice(["No Shuffle", "Basic Shuffle", "Bottles Shuffle"])
        )
        self.world_exit_var.set(
            choice(
                [
        ...
        # Within World Warps
        self.within_world_warps_var.set(
            choice(["No Shuffle", "Shuffle By World", "Shuffle By Game"])
        )
        # Starting World
        ...
        if (
            not (
                self.monster_house_var.get()
                or self.what_floor_var.get()
                or self.grunty_size_var.get()
            )
            and self.gruntilda_difficulty_var.get()
        ):
        ...
        # Treasure Trove Cove
        self.scattered_structs_var.set(
            choice(["No Scatter", "Low Scatter", "High Scatter"])
        )
        # Clanker's Cavern
    ...
    def _save_current_configuration(self, button_press=True):
        ...
        for enemy_name in master_enemy_dict:
            current_config[f"Include {enemy_name}"] = self.enemy_checkbox_dict[
                enemy_name
            ].get()
        # MAP Config
        for custom_name in self.map_config_checkbox_dict:
            current_config[f"Include {custom_name}"] = self.map_config_checkbox_dict[
                custom_name
            ].get()
        # Short Sounds
        for short_sound_type in self.short_sounds_dict:
            for short_sound_name in self.short_sounds_dict[short_sound_type]:
                current_config[f"Include {short_sound_name}"] = (
                    self.short_sounds_dict[short_sound_type][short_sound_name]
                ).get()
        # Jingles
        for jingle_name in self.jingles_dict:
            current_config[f"Include {jingle_name}"] = (
                self.jingles_dict[jingle_name]
            ).get()
        # Music
        for music_type in self.music_dict:
            for music_name in self.music_dict[music_type]:
                current_config[f"Include {music_name}"] = (
                    self.music_dict[music_type][music_name]
                ).get()
        if button_press:
            try:
                json_file = tkinter.filedialog.asksaveasfile(
                    filetypes=(("Json Files", "*.json"), ("all files", "*.*")),
    ...
    def _check_final_puzzle_value(self):
        ...
        if not final_puzzle_val.isdigit():
            Error_GUI(
                f"Final Puzzle Value Must Be An Integer: '{str(final_puzzle_val)}'"
            )
            return False
        ...
        if final_puzzle_val < 0:
            ...
            Error_GUI(
                "Final Puzzle Value Must Be Less Than 51 When Removing Floating Jiggies."
            )
            return False
        return True

    def _check_final_note_door_value(self) -> bool:
        """
        ...
        if not final_note_door_val.isdigit():
            Error_GUI(
                f"Final Note Door Value Must Be An Positive Integer: '{str(final_note_door_val)}'"
            )
            return False
        ...
        if (self.struct_var.get() == "All Notes") and (final_note_door_val > 2000):
            Error_GUI(
                "Final Note Door Value Must Be Less Than 2000 Under These Settings."
            )
            return False
        elif (self.struct_var.get() != "All Notes") and (final_note_door_val > 900):
            Error_GUI(
                "Final Note Door Value Must Be Less Than 900 Under These Settings."
            )
            return False
        return True
    ...
    def _check_cheato_values(self):
        ...
        for value in [
            self.before_blue_egg_carry_value.get(),
            self.after_blue_egg_carry_value.get(),
            self.before_red_feather_carry_value.get(),
            self.after_red_feather_carry_value.get(),
            self.before_gold_feather_carry_value.get(),
            self.after_gold_feather_carry_value.get(),
        ]:
            if value == "?":
                pass
            elif (not value.isdigit()) or (int(value) < 0) or (int(value) > 255):
                Error_GUI(
                    "Egg and Feather carrying capacities should be between 0 and 255."
                )
                return False
        return True
    ...
    def _check_starting_life_count(self):
        ...
        if (
            (not starting_lives.isdigit())
            or (int(self.starting_lives_entry.get()) < 0)
            or (int(self.starting_lives_entry.get()) > 255)
        ):
    ...
    def _submit(self):
        """
        If all input paramaters meet the requirements, we move onto actually
        randomizing the game
        """
        self.logger.info("Submitting Configuration")
        if (
            self._check_rom_directory()
            and self._check_seed_value()
            and self._check_final_note_door_value()
            and self._check_final_puzzle_value()
            and self._check_cheato_values()
            and self._check_starting_life_count()
        ):
            ...
            self.logger.debug(
                "At Least One Setting Seem To Be Invalid. Please check all settings."
            )
    ...
    def _main(self) -> None:
        ...
        self.ttp_image = tk.PhotoImage(file=(self.cwd / "Pictures" / "Tool_Tips.png"))
        ...
        self.rom_frame = tk.LabelFrame(
            self._general_tab,
            text="ROM File",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        self.rom_frame.pack(expand=tk.TRUE, fill=tk.BOTH)
        self.select_rom_button_ttp_canvas = tk.Label(
            self.rom_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.select_rom_button_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady
        )
        self.select_rom_button_ttp = self.CreateToolTip(
            self.select_rom_button_ttp_canvas,
            self,
            tool_tips_dict["ROM"]["SELECT_ROM_FILE"],
        )
        self.folder_image = tk.PhotoImage(
            file=(self.cwd / "Pictures" / "Open_File.png")
        )
        self.select_rom_button = tk.Button(
            self.rom_frame,
            ...
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
            command=self._select_rom_file,
        )
        self.select_rom_button.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.rom_file_entry = tk.StringVar(self.rom_frame)
        self.rom_file_display = tk.Entry(
            self.rom_frame,
            textvariable=self.rom_file_entry,
            state="readonly",
            width=35,
            font=(self.font_type, self.medium_font_size),
        )
        self.rom_file_display.grid(
            row=0, column=2, columnspan=2, padx=10, pady=self.pady
        )
        rom_file_disclaimer_text = (
            "ROMs must be v1.0 NTSC of Banjo-Kazooie, ending in .z64"
        )
        self.rom_file_disclaimer_label = tk.Label(
            self.rom_frame,
            text=rom_file_disclaimer_text,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.rom_file_disclaimer_label.grid(
            row=1, column=1, columnspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        ### Seed ###
        self.seed_frame = tk.LabelFrame(
            self._general_tab,
            text="Seed",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.random_seed_button_ttp_canvas = tk.Label(
            self.seed_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.random_seed_button_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady
        )
        self.random_seed_button_ttp = self.CreateToolTip(
            self.random_seed_button_ttp_canvas,
            self,
            tool_tips_dict["SEED"]["RANDOM_SEED_BUTTON"],
        )
        self._seed_image = tk.PhotoImage(file=(self.cwd / "Pictures" / "Seed.png"))
        self.random_seed_button = tk.Button(
            self.seed_frame,
            text="Select ROM File",
            command=self._random_seed,
            image=self._seed_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.random_seed_button.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        ...
        self.randomizer_setting_code_frame = tk.LabelFrame(
            self._general_tab,
            text="Randomizer Setting Code",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        self.randomizer_setting_code_frame.pack(expand=tk.TRUE, fill=tk.BOTH)
        self.randomizer_setting_code_ttp_canvas = tk.Label(
            self.randomizer_setting_code_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.randomizer_setting_code_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady
        )
        self.randomizer_setting_code_ttp = self.CreateToolTip(
            self.randomizer_setting_code_ttp_canvas,
            self,
            tool_tips_dict["SETTING_CODE"]["GENERATING_SETTING_CODE"],
        )
        self.generate_randomizer_setting_code_button = tk.Button(
            self.randomizer_setting_code_frame,
            text="Generate Settings Code",
            command=self._generate_randomizer_settings_code,
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.medium_font_size),
        )
        self.generate_randomizer_setting_code_button.grid(
            row=0, column=1, padx=10, pady=self.pady
        )
        self.apply_randomizer_setting_code_button = tk.Button(
            self.randomizer_setting_code_frame,
            text="Apply Settings Code",
            command=self._apply_randomizer_settings_code,
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.medium_font_size),
        )
        self.apply_randomizer_setting_code_button.grid(
            row=0, column=2, padx=10, pady=self.pady
        )
        self.randomizer_setting_code_value = tk.StringVar(
            self.randomizer_setting_code_frame
        )
        self.randomizer_setting_code_entry = tk.Entry(
            self.randomizer_setting_code_frame,
            textvariable=self.randomizer_setting_code_value,
            width=45,
            font=(self.font_type, self.medium_font_size),
        )
        self.randomizer_setting_code_entry.grid(
            row=1, column=0, columnspan=5, padx=10, pady=self.pady, sticky="w"
        )
        ### Random Settings ###
        self.random_settings_frame = tk.LabelFrame(
            self._general_tab,
        ...
        self.random_config_button = tk.Button(
            self.random_settings_frame,
            text="Randomly Select Configurations",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=(lambda: self._load_configuration(random_file=True)),
        )
        self.random_config_button.grid(row=0, column=0, padx=self.padx, pady=self.pady)
        self.random_settings_button = tk.Button(
            self.random_settings_frame,
            text="Randomly Select EVERY Setting",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._set_random_settings,
        )
        self.random_settings_button.grid(
        ...
        # Flagged Objects
        self.flagged_object_frame = tk.LabelFrame(
            self._collectables_tab,
            text="Jiggies, Empty Honeycombs, & Mumbo Tokens",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.flagged_object_frame.pack(expand=tk.TRUE, fill=tk.BOTH)
        self.flagged_object_ttp_canvas = tk.Label(
            self.flagged_object_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.flagged_object_ttp_canvas.grid(
            row=0, column=0, rowspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.flagged_object_ttp = self.CreateToolTip(
            self.flagged_object_ttp_canvas,
            self,
            tool_tips_dict["FLAGGED_OBJECTS"]["FRAME"],
        )
        self.flagged_object_var = tk.StringVar(self.flagged_object_frame)
        self.flagged_object_options = [
            "No Shuffle",
            "Shuffle (World)",
            "Shuffle (Game)",
        ]
        self.flagged_object_dropdown = ttk.Combobox(
            self.flagged_object_frame,
            textvariable=self.flagged_object_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        ...
        self.flagged_object_dropdown.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.flagged_object_abnormalities_var = tk.IntVar()
        self.flagged_object_abnormalities_checkbutton = tk.Checkbutton(
            self.flagged_object_frame,
            text="Include Abnormalities",
            variable=self.flagged_object_abnormalities_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.flagged_object_abnormalities_checkbutton.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.flagged_object_softlock_var = tk.IntVar()
        self.flagged_object_softlock_checkbutton = tk.Checkbutton(
            self.flagged_object_frame,
            text="Include Potential Softlocks",
            variable=self.flagged_object_softlock_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.flagged_object_softlock_checkbutton.grid(
            row=1, column=2, columnspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self._jiggy_image = tk.PhotoImage(file=(self.cwd / "Pictures" / "Jiggy.png"))
        self.random_puzzle_value_button = tk.Button(
            self.flagged_object_frame,
            text="Random Puzzle Value",
            command=self._random_puzzle_value,
            image=self._jiggy_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.random_puzzle_value_button.grid(
            row=2, column=2, rowspan=2, padx=self.padx, pady=self.pady
        )
        self.final_puzzle_ttp_canvas = tk.Label(
            self.flagged_object_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.final_puzzle_ttp_canvas.grid(
            row=2, column=0, rowspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.final_puzzle_checkbox_ttp = self.CreateToolTip(
            self.final_puzzle_ttp_canvas,
            self,
            tool_tips_dict["GRUNTILDAS_LAIR"]["FINAL_PUZZLE"],
        )
        self.final_puzzle_var = tk.IntVar()
        self.final_puzzle_checkbox = tk.Checkbutton(
            self.flagged_object_frame,
            text="Door Of Grunty Only",
            variable=self.final_puzzle_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.final_puzzle_checkbox.grid(
            row=2, column=1, rowspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.final_text = tk.Label(
            self.flagged_object_frame,
            text="Door of Grunty Jiggies:",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        ...
        self.final_puzzle_entry = tk.Entry(
            self.flagged_object_frame, textvariable=self.final_puzzle_value, width=6
        )
        self.final_puzzle_entry.grid(row=2, column=4, padx=self.padx, pady=self.pady)
        self.remove_floating_jiggies_var = tk.IntVar()
        self.remove_floating_jiggies_checkbox = tk.Checkbutton(
            self.flagged_object_frame,
            text="No Floating Jiggies",
            variable=self.remove_floating_jiggies_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.remove_floating_jiggies_checkbox.grid(
            row=3, column=3, padx=self.padx, pady=self.pady
        )
        self.max_health_banjo_var = tk.StringVar(self.flagged_object_frame)
        self.max_health_banjo_options = [
            "Normal Health",
            "Four Health Only",
            "Two Health Only",
            "One Health Only",
        ...
        self.max_health_banjo_dropdown = ttk.Combobox(
            self.flagged_object_frame,
            textvariable=self.max_health_banjo_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
            width=23,
        )
        self.max_health_banjo_dropdown.grid(
            row=4, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.max_health_banjo_dropdown["values"] = self.max_health_banjo_options
        self.max_health_banjo_dropdown["state"] = "readonly"
        self.free_transformations_text = tk.Label(
            self.flagged_object_frame,
            text="Token Option:",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.free_transformations_text.grid(
            row=4, column=2, padx=self.padx, pady=self.pady, sticky="e"
        )
        self.free_transformations_var = tk.StringVar(self.flagged_object_frame)
        self.free_transformations_options = [
            "Base Game Costs",
            "World Order Scaled Costs",
            "Free Transformations",
        ]
        self.free_transformations_dropdown = ttk.Combobox(
            self.flagged_object_frame,
            textvariable=self.free_transformations_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
            width=23,
        ...
        self.free_transformations_dropdown.grid(
            row=4, column=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.final_puzzle_var.trace("w", self._lock_final_puzzle_value)
        # Structs
        self.struct_frame = tk.LabelFrame(
            self._collectables_tab,
            text="Notes, Blue Eggs, Red Feathers, & Gold Feathers",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.struct_frame.pack(expand=tk.TRUE, fill=tk.BOTH)
        self.struct_ttp_canvas = tk.Label(
            self.struct_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.struct_ttp_canvas.grid(
            row=0, column=0, rowspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.struct_ttp = self.CreateToolTip(
            self.struct_ttp_canvas, self, tool_tips_dict["STRUCTS"]["FRAME"]
        )
        ...
        self.struct_dropdown = ttk.Combobox(
            self.struct_frame,
        ...
        self.struct_dropdown.grid(
            row=0, column=1, rowspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.note_overflow_var = tk.StringVar(self.struct_frame)
        self.note_overflow_options = [
            "Allow Save & Quit/Reset",
            "Possible No Save & Quit/Reset",
        ]
        self.note_overflow_dropdown = ttk.Combobox(
            self.struct_frame,
            textvariable=self.note_overflow_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
            width=26,
        ...
        self.note_overflow_dropdown.grid(
            row=0, column=2, columnspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.struct_note_count_var = tk.StringVar(self.struct_frame)
        self.struct_note_count_options = [
            "Produce Extra Notes",
            "Produce Exactly Enough Notes",
        ]
        self.struct_note_count_dropdown = ttk.Combobox(
            self.struct_frame,
            textvariable=self.struct_note_count_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
            width=26,
        ...
        self.final_note_door_ttp_canvas = tk.Label(
            self.struct_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.final_note_door_ttp_canvas.grid(
            row=2, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.final_note_door_checkbox_ttp = self.CreateToolTip(
            self.final_note_door_ttp_canvas,
            self,
            tool_tips_dict["GRUNTILDAS_LAIR"]["FINAL_NOTE_DOOR"],
        )
        self.final_note_door_var = tk.StringVar(self.struct_frame)
        self.note_door_options = ["Scaling Note Doors", "Final Note Door Only"]
        self.final_note_door_dropdown = ttk.Combobox(
            self.struct_frame,
            textvariable=self.final_note_door_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        ...
        self.final_note_door_dropdown.grid(
            row=2, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.final_note_door_value = tk.StringVar(self.struct_frame)
        self._note_image = tk.PhotoImage(file=(self.cwd / "Pictures" / "Note.png"))
        self.random_note_value_button = tk.Button(
            self.struct_frame,
            text="Random Note Value",
            command=self._random_note_value,
            image=self._note_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.random_note_value_button.grid(
            row=2, column=2, padx=self.padx, pady=self.pady
        )
        self.final_text = tk.Label(
            self.struct_frame,
            text="810 Note Door Value:",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.final_text.grid(row=2, column=3, padx=self.padx, pady=self.pady)
        self.final_note_door_entry = tk.Entry(
            self.struct_frame, textvariable=self.final_note_door_value, width=6
        )
        self.final_note_door_entry.grid(
            row=2, column=4, padx=self.padx, pady=self.pady, sticky="e"
        )
        self.final_text = tk.Label(
            self.struct_frame,
            text="Notes",
        ...
        self.carry_limit_frame = tk.LabelFrame(
            self.struct_frame,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.carry_limit_frame.grid(
            row=3, column=0, columnspan=6, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.carry_limit_frame["borderwidth"] = 0
        self.carry_limit_frame["highlightthickness"] = 0
        self.carry_limit_ttp_canvas = tk.Label(
            self.carry_limit_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.carry_limit_ttp_canvas.grid(
            row=0, column=0, rowspan=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.carry_limit_ttp = self.CreateToolTip(
            self.carry_limit_ttp_canvas, self, tool_tips_dict["STRUCTS"]["CARRY_LIMIT"]
        )
        self.final_text = tk.Label(
            self.carry_limit_frame,
            text="Before Cheato",
        ...
        self.blue_egg_carry_entry = tk.Entry(
            self.carry_limit_frame,
            textvariable=self.before_blue_egg_carry_value,
            width=6,
        )
        ...
        self.blue_egg_carry_entry = tk.Entry(
            self.carry_limit_frame,
            textvariable=self.after_blue_egg_carry_value,
            width=6,
        )
        ...
        self.red_feather_carry_entry = tk.Entry(
            self.carry_limit_frame,
            textvariable=self.before_red_feather_carry_value,
            width=6,
        )
        self.red_feather_carry_entry.grid(
            row=1, column=3, padx=self.padx, pady=self.pady
        )
        self.after_red_feather_carry_value = tk.StringVar(self.struct_frame)
        self.red_feather_carry_entry = tk.Entry(
            self.carry_limit_frame,
            textvariable=self.after_red_feather_carry_value,
            width=6,
        )
        self.red_feather_carry_entry.grid(
            row=2, column=3, padx=self.padx, pady=self.pady
        )
        self.before_gold_feather_carry_value = tk.StringVar(self.struct_frame)
        self.gold_feather_carry_entry = tk.Entry(
            self.carry_limit_frame,
            textvariable=self.before_gold_feather_carry_value,
            width=6,
        )
        self.gold_feather_carry_entry.grid(
            row=1, column=4, padx=self.padx, pady=self.pady
        )
        self.after_gold_feather_carry_value = tk.StringVar(self.struct_frame)
        self.gold_feather_carry_entry = tk.Entry(
            self.carry_limit_frame,
            textvariable=self.after_gold_feather_carry_value,
            width=6,
        )
        self.gold_feather_carry_entry.grid(
            row=2, column=4, padx=self.padx, pady=self.pady
        )
        self.random_carry_capacity_button = tk.Button(
            self.carry_limit_frame,
            text="Random Carry\nCapacities",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._set_random_carry_capacities,
        )
        self.random_carry_capacity_button.grid(
            row=1, rowspan=2, column=5, padx=self.padx, pady=self.pady
        )
        self.struct_var.trace("w", self._lock_struct_options)
        # Non Flagged Objects
        self.non_flagged_object_frame = tk.LabelFrame(
            self._collectables_tab,
            text="Jinjos, 1-Ups, & Misc Objects",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.non_flagged_object_frame.pack(expand=tk.TRUE, fill=tk.BOTH)
        self.non_flagged_object_ttp_canvas = tk.Label(
            self.non_flagged_object_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.non_flagged_object_ttp_canvas.grid(
            row=0, column=0, rowspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.non_flagged_object_ttp = self.CreateToolTip(
            self.non_flagged_object_ttp_canvas,
            self,
            tool_tips_dict["NON_FLAGGED_OBJECTS"]["FRAME"],
        )
        ...
        self.non_flagged_object_dropdown = ttk.Combobox(
            self.non_flagged_object_frame,
            textvariable=self.non_flagged_object_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        ...
        self.non_flagged_object_dropdown.grid(
            row=0, column=1, columnspan=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        ...
        self.jinjo_color_dropdown = ttk.Combobox(
            self.non_flagged_object_frame,
            textvariable=self.jinjo_color_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        ...
        self.jinjo_color_dropdown.grid(
            row=0, column=2, columnspan=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.non_flagged_object_abnormalities_var = tk.IntVar()
        self.non_flagged_object_abnormalities_checkbutton = tk.Checkbutton(
            self.non_flagged_object_frame,
            text="Include Abnormalities (May Include Eggs and Feathers)",
            variable=self.non_flagged_object_abnormalities_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.non_flagged_object_abnormalities_checkbutton.grid(
            row=1, column=1, columnspan=3, padx=self.padx, pady=self.pady, sticky="sw"
        )
        self.starting_lives_text = tk.Label(
            self.non_flagged_object_frame,
            text="Starting Life Count:",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.starting_lives_text.grid(
            row=2, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.starting_lives_value = tk.StringVar(self.struct_frame)
        self.starting_lives_entry = tk.Entry(
            self.non_flagged_object_frame,
            textvariable=self.starting_lives_value,
            width=6,
        )
        self.starting_lives_entry.grid(
            row=2, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        #################
        ### WARPS TAB ###
        #################
        ...
        # World Entrances
        self.world_entrance_frame = tk.LabelFrame(
            self._warps_tab,
            text="World Entrances (Includes Randomized Moves)",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.world_entrance_frame.pack(expand=tk.TRUE, fill=tk.BOTH)
        self.world_entrance_ttp_canvas = tk.Label(
            self.world_entrance_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.world_entrance_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.world_entrance_ttp = self.CreateToolTip(
            self.world_entrance_ttp_canvas,
            self,
            tool_tips_dict["WORLD_ENTRANCES"]["FRAME"],
        )
        ...
        self.world_entrance_dropdown = ttk.Combobox(
            self.world_entrance_frame,
            textvariable=self.world_entrance_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        ...
        self.world_entrance_dropdown.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.world_exit_var = tk.StringVar(self.world_entrance_frame)
        self.world_exit_options = [
            "Exit From World You Were Just In",
            "Exit From Entrance You Entered From",
        ]
        self.world_exit_dropdown = ttk.Combobox(
            self.world_entrance_frame,
            textvariable=self.world_exit_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
            width=33,
        ...
        self.world_exit_dropdown.grid(
            row=0, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.all_starting_moves_var = tk.IntVar()
        self.all_starting_moves_checkbutton = tk.Checkbutton(
            self.world_entrance_frame,
            text="Start Game With All Moves",
            variable=self.all_starting_moves_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.all_starting_moves_checkbutton.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="sw"
        )
        # Within World Warps
        self.within_world_warp_frame = tk.LabelFrame(
            self._warps_tab,
            text="Within The World Warps",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.within_world_warp_frame.pack(expand=tk.TRUE, fill=tk.BOTH)
        self.within_world_warp_ttp_canvas = tk.Label(
            self.within_world_warp_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.within_world_warp_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.within_world_warp_ttp = self.CreateToolTip(
            self.within_world_warp_ttp_canvas,
            self,
            tool_tips_dict["WITHIN_WORLD_WARPS"]["FRAME"],
        )
        self.within_world_warps_var = tk.StringVar(self.within_world_warp_frame)
        self.within_world_warps_options = [
            "No Shuffle",
            "Shuffle By World",
            "Shuffle By Game",
        ]
        self.within_world_warps_dropdown = ttk.Combobox(
            self.within_world_warp_frame,
            textvariable=self.within_world_warps_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        ...
        self.within_world_warps_dropdown.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        ...
        self.warp_disclaimer_label = tk.Label(
            self.within_world_warp_frame,
            text=warp_disclaimer_text,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, 12),
            anchor="w",
        ...
        self.warp_disclaimer_label.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        # Starting Area
        self.starting_area_frame = tk.LabelFrame(
            self._warps_tab,
            text="Starting Area:",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.starting_area_ttp_canvas = tk.Label(
            self.starting_area_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.starting_area_ttp_canvas.grid(
            row=0, column=0, rowspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        ...
        self.starting_area_label = tk.Label(
            self.starting_area_frame,
            text=starting_area_disclaimer_text,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, 12),
            anchor="w",
        ...
        self.starting_area_label.grid(
            row=0, column=1, columnspan=4, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.new_area_ttp = self.CreateToolTip(
            self.starting_area_ttp_canvas,
            self,
            tool_tips_dict["STARTING_AREA"]["NEW_GAME"],
        )
        self.new_area_text = tk.Label(
            self.starting_area_frame,
            text="Starting Area",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.new_area_text.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.new_area_var = tk.StringVar(self.world_entrance_frame)
        self.starting_area_options = [option for option in start_level_ids]
        self.starting_area_options.insert(
            0, "Random Starting Area (Auto Have All Moves)"
        )
        self.new_area_dropdown = ttk.Combobox(
            self.starting_area_frame,
            textvariable=self.new_area_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
            width=39,
        ...
        self.new_area_dropdown.grid(
            row=1, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.skip_intro_cutscenes_var = tk.IntVar()
        self.skip_intro_cutscene_checkbutton = tk.Checkbutton(
            self.starting_area_frame,
            text="Skip Intro Cutscenes",
            variable=self.skip_intro_cutscenes_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.skip_intro_cutscene_checkbutton.grid(
            row=2, column=1, columnspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.default_starting_area_button = tk.Button(
            self.starting_area_frame,
            text="Default\nStarting Area",
            command=self._default_starting_area,
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
        )
        self.default_starting_area_button.grid(
            row=1, column=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        ...
        # Enemies
        self.enemies_frame = tk.LabelFrame(
            self._enemies_tab,
            text="Ground, Wall, & Flying Enemies",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        ...
        self.enemies_ttp_canvas = tk.Label(
            self.enemies_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.enemies_ttp_canvas.grid(
            row=0, column=0, rowspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.enemies_ttp = self.CreateToolTip(
            self.enemies_ttp_canvas, self, tool_tips_dict["ENEMIES"]["FRAME"]
        )
        self.enemies_var = tk.StringVar(self.enemies_frame)
        self.enemies_options = ["Default Enemies", "Shuffle", "Randomize"]
        self.enemies_dropdown = ttk.Combobox(
            self.enemies_frame,
        ...
        self.enemy_size_dropdown = ttk.Combobox(
            self.enemies_frame,
            textvariable=self.enemy_size_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
            width=23,
        ...
        self.enemy_size_dropdown.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.non_softlock_enemies_button = tk.Button(
            self.enemies_frame,
            text="Select All\nNon-Softlock Enemies",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=(lambda: self._select_non_softlock_enemies()),
        )
        self.non_softlock_enemies_button.grid(
            row=0, column=2, rowspan=2, padx=self.padx, pady=self.pady, sticky="e"
        )
        self.clear_enemies_button = tk.Button(
            self.enemies_frame,
            text="Remove All\nEnemies",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=(lambda: self._remove_all_enemies()),
        )
        self.clear_enemies_button.grid(
            row=0, column=3, rowspan=2, padx=self.padx, pady=self.pady, sticky="e"
        )
        self.enemy_checklist_frame = tk.LabelFrame(
            self.enemies_frame,
            text="Enemies To Include In Randomization:",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.enemy_checklist_frame.grid(
            row=2, column=0, columnspan=6, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.enemy_checklist_frame["borderwidth"] = 0
        self.enemy_checklist_frame["highlightthickness"] = 0
        self.softlock_enemies_text = tk.Label(
            self.enemy_checklist_frame,
            text="WARNING: Enemies with * may softlock/crash the game. Check the box at your own risk.",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.softlock_enemies_text.grid(
            row=0, column=0, columnspan=6, padx=self.padx, pady=self.pady, sticky="w"
        )
        ...
        for enemy_count, enemy_name in enumerate(sorted(master_enemy_dict)):
            ...
            enemy_checkbutton = tk.Checkbutton(
                self.enemy_checklist_frame,
                text=enemy_name,
                variable=self.enemy_checkbox_dict[enemy_name],
                foreground=self.black,
                background=curr_background_color,
                font=(self.font_type, self.small_font_size),
                width=13,
            ...
            enemy_checkbutton.grid(
                row=(enemy_count // 4) + 1,
                column=(enemy_count % 4),
                padx=self.padx,
                pady=self.pady,
                sticky="w",
            )
        ...
        # BK Model
        self.bk_model_frame = tk.LabelFrame(
            self._bk_model_tab,
            text="Banjo-Kazooie Model Color",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.bk_model_frame.pack(expand=tk.TRUE, fill=tk.BOTH)
        self.bk_model_ttp_canvas = tk.Label(
            self.bk_model_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.bk_model_ttp_canvas.grid(
            row=0, rowspan=2, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.bk_model_frame_ttp = self.CreateToolTip(
            self.bk_model_ttp_canvas, self, tool_tips_dict["BK_COLOR"]["FRAME"]
        )
        self.bk_model_json = read_json(
            self.cwd
            / "Randomization_Processes"
            / "Misc_Manipulation"
            / "Model_Data"
        ...
        self.bk_model_dropdown["state"] = "readonly"
        self.bk_model_dropdown.grid(
            row=0, column=1, columnspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.random_bk_model_preset_button = tk.Button(
            self.bk_model_frame,
            text="Random Preset",
            command=self._random_bk_model_preset,
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
        )
        self.random_bk_model_preset_button.grid(
            row=0, column=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.delete_bk_model_preset_button = tk.Button(
            self.bk_model_frame,
            text="Delete Preset",
            command=self._delete_bk_colors,
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
        )
        self.delete_bk_model_preset_button.grid(
            row=1, column=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        if self.bk_model_var.get() in [
            "Seed Determined Preset",
            "Seed Determined Colors",
        ]:
            self.bk_model_image = tk.PhotoImage(
                file=(self.cwd / "Pictures" / "BK_Models" / "Default.png")
            )
        else:
            self.bk_model_image = tk.PhotoImage(
                file=(
                    ...
                    / "Pictures"
                    / "BK_Models"
                    / f"{self.bk_model_var.get()}.png"
                )
            )
        self.bk_model_image_label = tk.Label(
            self.bk_model_frame,
            text="Preview",
            foreground=curr_background_color,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
            image=self.bk_model_image,
        )
        self.bk_model_image_label.grid(
            row=0, rowspan=2, column=4, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.rgb32_text = tk.Label(
            self.bk_model_frame,
            text="RGB32",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.rgb32_text.grid(
            row=2, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.rgb16_text = tk.Label(
            self.bk_model_frame,
            text="RGB16",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.banjo_fur_text.grid(
            row=3, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.banjo_fur_var = tk.StringVar(self.bk_model_frame)
        self.banjo_fur_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.banjo_fur_var, width=9
        )
        self.banjo_fur_entry.grid(
            row=3, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.banjo_skin_text = tk.Label(
            self.bk_model_frame,
            text="Banjo's Skin",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.banjo_skin_text.grid(
            row=4, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.banjo_skin_var = tk.StringVar(self.bk_model_frame)
        self.banjo_skin_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.banjo_skin_var, width=9
        )
        self.banjo_skin_entry.grid(
            row=4, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.banjo_feet_text = tk.Label(
            self.bk_model_frame,
            text="Banjo's Toes",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.banjo_feet_text.grid(
            row=4, column=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.banjo_feet_var = tk.StringVar(self.bk_model_frame)
        self.banjo_feet_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.banjo_feet_var, width=5
        )
        self.banjo_feet_entry.grid(
            row=4, column=4, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.kazooie_primary_text = tk.Label(
            self.bk_model_frame,
            text="Kazooie Primary",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.kazooie_primary_text.grid(
            row=5, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.kazooie_primary_var = tk.StringVar(self.bk_model_frame)
        self.kazooie_primary_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.kazooie_primary_var, width=9
        )
        self.kazooie_primary_entry.grid(
            row=5, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.kazooie_wing_primary_text = tk.Label(
            self.bk_model_frame,
            text="Wing Primary",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.kazooie_wing_primary_text.grid(
            row=5, column=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.kazooie_wing_primary_var = tk.StringVar(self.bk_model_frame)
        self.kazooie_wing_primary_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.kazooie_wing_primary_var, width=5
        )
        self.kazooie_wing_primary_entry.grid(
            row=5, column=4, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.kazooie_secondary_text = tk.Label(
            self.bk_model_frame,
            text="Kazooie Secondary",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.small_font_size),
        )
        self.kazooie_secondary_text.grid(
            row=6, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.kazooie_secondary_var = tk.StringVar(self.bk_model_frame)
        self.kazooie_secondary_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.kazooie_secondary_var, width=9
        )
        self.kazooie_secondary_entry.grid(
            row=6, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.kazooie_wing_secondary_text = tk.Label(
            self.bk_model_frame,
            text="Wing Secondary",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.kazooie_wing_secondary_text.grid(
            row=6, column=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.kazooie_wing_secondary_var = tk.StringVar(self.bk_model_frame)
        self.kazooie_wing_secondary_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.kazooie_wing_secondary_var, width=5
        )
        self.kazooie_wing_secondary_entry.grid(
            row=6, column=4, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.shorts_vertex_text = tk.Label(
            self.bk_model_frame,
            text="Shorts Main",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.shorts_vertex_text.grid(
            row=7, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.shorts_vertex_var = tk.StringVar(self.bk_model_frame)
        self.shorts_vertex_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.shorts_vertex_var, width=9
        )
        self.shorts_vertex_entry.grid(
            row=7, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.shorts_texture_text = tk.Label(
            self.bk_model_frame,
            text="Shorts Front",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.shorts_texture_text.grid(
            row=7, column=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.shorts_texture_var = tk.StringVar(self.bk_model_frame)
        self.shorts_texture_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.shorts_texture_var, width=5
        )
        self.shorts_texture_entry.grid(
            row=7, column=4, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.tooth_necklace_text = tk.Label(
            self.bk_model_frame,
            text="Tooth Necklace",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.tooth_necklace_text.grid(
            row=8, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.tooth_necklace_var = tk.StringVar(self.bk_model_frame)
        self.tooth_necklace_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.tooth_necklace_var, width=9
        )
        self.tooth_necklace_entry.grid(
            row=8, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.backpack_text = tk.Label(
            self.bk_model_frame,
            text="Backpack",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.backpack_text.grid(
            row=9, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.backpack_var = tk.StringVar(self.bk_model_frame)
        self.backpack_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.backpack_var, width=9
        )
        self.backpack_entry.grid(
            row=9, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.wading_boots_text = tk.Label(
            self.bk_model_frame,
            text="Wading Boots",
            foreground=self.black,
            background=curr_background_color,
        ...
        self.wading_boots_text.grid(
            row=10, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.wading_boots_var = tk.StringVar(self.bk_model_frame)
        self.wading_boots_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.wading_boots_var, width=9
        )
        self.wading_boots_entry.grid(
            row=10, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.transfer_32_to_16_button = tk.Button(
            self.bk_model_frame,
            text="Transfer RGB32\nTo RGB16 Parts",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._transfer_rgb32_to_rgb16,
        )
        self.transfer_32_to_16_button.grid(
            row=9, rowspan=2, column=3, columnspan=2, padx=self.padx, pady=self.pady
        )
        self.bk_model_frame.grid_rowconfigure(11, minsize=20)
        self.custom_bk_model_name_label = tk.Label(
            self.bk_model_frame,
            text="Custom Colors Name:",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
            anchor="w",
        ...
        self.custom_bk_model_name_label.grid(
            row=12, column=2, columnspan=2, padx=self.padx, pady=self.pady
        )
        ...
        self.custom_bk_model_name_entry = tk.Entry(
            self.bk_model_frame, textvariable=self.custom_bk_model_name_var, width=20
        )
        self.custom_bk_model_name_entry.grid(
            row=13, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.save_bk_model_preset_button = tk.Button(
            self.bk_model_frame,
            text="Save As Preset",
            command=self._save_bk_colors,
            foreground=self.white,
            background=self.blue,
            font=(self.font_type, self.small_font_size),
        )
        self.save_bk_model_preset_button.grid(
            row=13, column=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        ...
        self._sounds_tab_control = ttk.Notebook(
            self._sounds_tab, width=self.app_window.winfo_width()
        )
        music_dict = read_json(
            f"{os.getcwd()}/Randomization_Processes/Misc_Manipulation/Music_Data/BK_Sounds.json"
        )
        # Short Sounds
        ...
        self.short_sounds_frame = tk.LabelFrame(
            self._short_sounds_tab,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        ...
        self.short_sounds_label = tk.Label(
            self.short_sounds_frame,
            text="Short sounds are any one second noises, like collecting items.\nJarring sounds are harsher sounding sounds.",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, 12),
            anchor="w",
        ...
        self.short_sounds_label.grid(
            row=0, column=0, columnspan=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.short_sounds_select_all_button = tk.Button(
            self.short_sounds_frame,
            text="Select All\nShort Sounds",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._select_all_short_sounds,
        )
        self.short_sounds_select_all_button.grid(row=1, column=0, padx=self.padx)
        self.short_sounds_select_non_jarring_button = tk.Button(
            self.short_sounds_frame,
            text="Select All\nNon-Jarring\nShort Sounds",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._select_non_jarring_short_sounds,
        )
        self.short_sounds_select_non_jarring_button.grid(
            row=1, column=1, padx=self.padx
        )
        self.short_sounds_deselect_all_button = tk.Button(
            self.short_sounds_frame,
            text="Deselect All\nShort Sounds",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._deselect_all_short_sounds,
        )
        ...
        self._short_sounds_tab_control = ttk.Notebook(
            self._short_sounds_tab, width=self.app_window.winfo_width()
        )
        for short_sound_type in self.short_sounds_dict:
            self.music_type_tabs[short_sound_type] = {}
            self.music_type_tabs[short_sound_type]["Tab"] = ttk.Frame(
                self._short_sounds_tab
            )
            self._short_sounds_tab_control.add(
                self.music_type_tabs[short_sound_type]["Tab"], text=short_sound_type
            )
            self.music_type_tabs[short_sound_type]["Frame"] = tk.LabelFrame(
                self.music_type_tabs[short_sound_type]["Tab"],
                foreground=self.black,
                background=curr_background_color,
                font=(self.font_type, self.medium_font_size),
            )
            (self.music_type_tabs[short_sound_type]["Frame"]).pack(
            ...
            for short_sound_count, short_sound_name in enumerate(
                sorted(self.short_sounds_dict[short_sound_type])
            ):
                short_sound_checkbutton = tk.Checkbutton(
                    self.music_type_tabs[short_sound_type]["Frame"],
                    text=short_sound_name,
                    variable=self.short_sounds_dict[short_sound_type][short_sound_name],
                    foreground=self.black,
                    background=curr_background_color,
                    font=(self.font_type, self.small_font_size),
                    width=20,
        ...
        self.jingles_label = tk.Label(
            self.jingles_frame,
            text="Jingles/Fanfare are any several second songs, like collecting a Jiggy.",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, 12),
            anchor="w",
        ...
        self.jingles_label.grid(
            row=0, column=0, columnspan=3, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.jingles_select_all_button = tk.Button(
            self.jingles_frame,
            text="Select All\nJingles/Fanfare",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._select_all_jingles,
        )
        self.jingles_select_all_button.grid(row=1, column=0, padx=self.padx)
        self.jingles_deselect_all_button = tk.Button(
            self.jingles_frame,
            text="Deselect All\nJingles/Fanfare",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._deselect_all_jingles,
        )
        ...
        for jingle_count, jingle_pointer in enumerate(sorted(music_dict["Jingle"])):
            ...
            jingle_checkbutton = tk.Checkbutton(
                self.jingles_frame,
                text=jingle_name,
                variable=self.jingles_dict[jingle_name],
                foreground=self.black,
                background=curr_background_color,
                font=(self.font_type, self.small_font_size),
                width=20,
            ...
            jingle_checkbutton.grid(
                row=(jingle_count // 3) + 2,
                column=(jingle_count % 3),
                padx=self.padx,
                pady=self.pady,
                sticky="w",
            )
        ...
        self.music_frame = tk.LabelFrame(
            self._music_tab,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        ...
        self.music_label = tk.Label(
            self.music_frame,
            text="Music are songs that go on for a while, like level songs.",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, 12),
            anchor="w",
        ...
        self.music_label.grid(
            row=0, column=0, columnspan=4, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.music_select_all_button = tk.Button(
            self.music_frame,
            text="Select All\nMusic",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._select_all_music,
        )
        self.music_select_all_button.grid(row=1, column=0, padx=self.padx)
        self.music_deselect_all_button = tk.Button(
            self.music_frame,
            text="Deselect All\nMusic",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=self._deselect_all_music,
        )
        ...
        self.music_type_tabs = {}
        self._music_tab_control = ttk.Notebook(
            self._music_tab, width=self.app_window.winfo_width()
        )
        for music_type in self.music_dict:
            self.music_type_tabs[music_type] = {}
            self.music_type_tabs[music_type]["Tab"] = ttk.Frame(self._music_tab)
            self._music_tab_control.add(
                self.music_type_tabs[music_type]["Tab"], text=music_type
            )
            self.music_type_tabs[music_type]["Frame"] = tk.LabelFrame(
                self.music_type_tabs[music_type]["Tab"],
                foreground=self.black,
                background=curr_background_color,
                font=(self.font_type, self.medium_font_size),
            )
            (self.music_type_tabs[music_type]["Frame"]).pack(
                expand=tk.TRUE, fill=tk.BOTH
            )
            (self.music_type_tabs[music_type]["Frame"])["borderwidth"] = 0
            (self.music_type_tabs[music_type]["Frame"])["highlightthickness"] = 0
            for music_count, music_name in enumerate(
                sorted(self.music_dict[music_type])
            ):
                music_checkbutton = tk.Checkbutton(
                    self.music_type_tabs[music_type]["Frame"],
                    text=music_name,
                    variable=self.music_dict[music_type][music_name],
                    foreground=self.black,
                    background=curr_background_color,
                    font=(self.font_type, self.small_font_size),
                    width=20,
                ...
                music_checkbutton.grid(
                    row=(music_count // 3) + 1, column=(music_count % 3), sticky="w"
                )
        #########################################
        ...
        self.map_config_frame = tk.LabelFrame(
            self._custom_settings_tab,
            text="Models, Animations, & Properties",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        ...
        self.map_config_disclaimer_label = tk.Label(
            self.map_config_frame,
            text=map_config_disclaimer_text,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, 12),
            anchor="w",
        ...
        self.map_config_disclaimer_label.grid(
            row=0, column=0, columnspan=4, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.map_config_ttp_canvas = tk.Label(
            self.map_config_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.map_config_ttp_canvas.grid(
            row=1, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.map_config_frame_ttp = self.CreateToolTip(
            self.map_config_ttp_canvas, self, tool_tips_dict["MAP_CONFIG"]["FRAME"]
        )
        self.all_aesthetics_button = tk.Button(
            self.map_config_frame,
            text="Select All\nCustom Aesthetic",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=(lambda: self._all_custom_aesthetics()),
        )
        self.all_aesthetics_button.grid(row=1, column=1, padx=self.padx, pady=self.pady)
        self.no_customization_button = tk.Button(
            self.map_config_frame,
            text="Uncheck All\nCustom Options",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=(lambda: self._no_customization()),
        )
        self.no_customization_button.grid(
            row=1, column=2, padx=self.padx, pady=self.pady
        )
        self.random_customization_button = tk.Button(
            self.map_config_frame,
            text="Random Customs\nAnd Hide Options",
            foreground=self.white,
            background=self.red,
            font=(self.font_type, self.small_font_size),
            command=(lambda: self._random_customization()),
            width=15,
        )
        self.random_customization_button.grid(
            row=1, column=3, padx=self.padx, pady=self.pady
        )
        self.map_config_checklist_frame = tk.LabelFrame(
            self.map_config_frame,
            text="(A) = Aesthetical Only; (P) = Contains Property Changes",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.map_config_checklist_frame.grid(
            row=2, column=0, columnspan=6, padx=self.padx, pady=self.pady, sticky="w"
        )
        ...
        self.hiding_customization = False
        self.customization_checkbuttons = []
        for json_count, json_name in enumerate(
            sorted(
                os.listdir(
                    self.cwd
                    / "Randomization_Processes/Misc_Manipulation/Models_Animations_Properties/JSON_Files/"
                )
            )
        ):
            display_name = json_name.split(".json")[0]
            self.map_config_checkbox_dict[display_name] = tk.IntVar()
            custom_checkbutton = tk.Checkbutton(
                self.map_config_checklist_frame,
                text=display_name,
                variable=self.map_config_checkbox_dict[display_name],
                foreground=self.black,
                background=curr_background_color,
                font=(self.font_type, self.small_font_size),
                width=13,
        ...
        self._world_specific_tab_control = ttk.Notebook(
            self._world_specific_tab, width=self.app_window.winfo_width()
        )
        # Spiral Mountain
        # Gruntilda's Lair
        curr_background_color = "#660066"
        self._gruntildas_lair_tab = ttk.Frame(
            self._world_specific_tab_control, width=10
        )
        self._world_specific_tab_control.add(self._gruntildas_lair_tab, text="GL")
        self.gruntildas_lair_frame = tk.LabelFrame(
            self._gruntildas_lair_tab,
            text="Gruntilda's Lair",
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        ...
        self.skip_furnace_fun_ttp_canvas = tk.Label(
            self.gruntildas_lair_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        )
        self.skip_furnace_fun_ttp_canvas.grid(
            row=0, column=0, rowspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.skip_furnace_fun_checkbox_ttp = self.CreateToolTip(
            self.skip_furnace_fun_ttp_canvas,
            self,
            tool_tips_dict["GRUNTILDAS_LAIR"]["SKIP_FURNACE_FUN"],
        )
        self.skip_furnace_fun_var = tk.IntVar()
        self.skip_furnace_fun_checkbox = tk.Checkbutton(
            self.gruntildas_lair_frame,
            text="Skip Furnace Fun",
            variable=self.skip_furnace_fun_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.skip_furnace_fun_checkbox.grid(
            row=0, column=1, columnspan=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.brentilda_hints_var = tk.StringVar(self.gruntildas_lair_frame)
        self.brentilda_hints_options = [
            "Base Game Brentilda Hints",
            "Vague Brentilda Rando Hints",
            "Detailed Brentilda Rando Hints",
        ]
        self.brentilda_hints_dropdown = ttk.Combobox(
            self.gruntildas_lair_frame,
            textvariable=self.brentilda_hints_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
            width=24,
        ...
        self.remove_magic_barriers_ttp_canvas = tk.Label(
            self.gruntildas_lair_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.remove_magic_barriers_ttp_canvas.grid(
            row=2, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.remove_magic_barriers_checkbox_ttp = self.CreateToolTip(
            self.remove_magic_barriers_ttp_canvas,
            self,
            tool_tips_dict["GRUNTILDAS_LAIR"]["NO_DETRANSFORMATIONS"],
        )
        self.remove_magic_barriers_var = tk.IntVar()
        self.remove_magic_barriers_checkbox = tk.Checkbutton(
            self.gruntildas_lair_frame,
            text="No Detransformations",
            variable=self.remove_magic_barriers_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.remove_magic_barriers_checkbox.grid(
            row=2, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.gruntilda_difficulty_ttp_canvas = tk.Label(
            self.gruntildas_lair_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.gruntilda_difficulty_ttp_canvas.grid(
            row=3, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.gruntilda_difficulty_checkbox_ttp = self.CreateToolTip(
            self.gruntilda_difficulty_ttp_canvas,
            self,
            tool_tips_dict["GRUNTILDAS_LAIR"]["HARDER_FINAL_BATTLE"],
        )
        self.gruntilda_difficulty_text = tk.Label(
            self.gruntildas_lair_frame,
            text="Final Battle Difficulty?\n0 For Default; 3 For Hard",
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.gruntilda_difficulty_text.grid(
            row=3, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.gruntilda_difficulty_var = tk.IntVar()
        self.gruntilda_difficulty_scale = tk.Scale(
            self.gruntildas_lair_frame,
            from_=0,
            to=3,
            orient=tkinter.HORIZONTAL,
            variable=self.gruntilda_difficulty_var,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.gruntilda_difficulty_scale.grid(
            row=3, column=2, columnspan=2, padx=self.padx, pady=self.pady, sticky="n"
        )
        self.monster_house_var = tk.IntVar()
        self.monster_house_checkbox = tk.Checkbutton(
            self.gruntildas_lair_frame,
            text="Monster House",
            variable=self.monster_house_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        ...
        self.what_floor_checkbox = tk.Checkbutton(
            self.gruntildas_lair_frame,
            text="What Floor?",
            variable=self.what_floor_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        ...
        self.grunty_size_checkbox = tk.Checkbutton(
            self.gruntildas_lair_frame,
            text="Mini Me",
            variable=self.grunty_size_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        ...
        self.mumbos_mountain_frame = tk.LabelFrame(
            self._mumbos_mountain_tab,
            text="Mumbo's Mountain",
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        ...
        self.flowers_ttp_canvas = tk.Label(
            self.mumbos_mountain_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.flowers_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.flowers_checkbox_ttp = self.CreateToolTip(
            self.flowers_ttp_canvas,
            self,
            tool_tips_dict["MUMBOS_MOUNTAIN"]["INCLUDE_FLOWERS"],
        )
        self.flowers_var = tk.IntVar()
        self.flowers_checkbox = tk.Checkbutton(
            self.mumbos_mountain_frame,
            text="Include Flowers (Shuffling/Randomizing)",
            variable=self.flowers_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.flowers_checkbox.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        # Treasure Trove Cove
        ...
        self.treasure_trove_cove_frame = tk.LabelFrame(
            self._treasure_trove_cove_tab,
            text="Treasure Trove Cove",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        ...
        self.scattered_structs_ttp_canvas = tk.Label(
            self.treasure_trove_cove_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        self.scattered_structs_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.scattered_structs_checkbox_ttp = self.CreateToolTip(
            self.scattered_structs_ttp_canvas,
            self,
            tool_tips_dict["TREASURE_TROVE_COVE"]["SCATTERED_STRUCTS"],
        )
        self.scattered_structs_text = tk.Label(
            self.treasure_trove_cove_frame,
            text="Scatter Notes/Eggs/Feathers:",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.scattered_structs_text.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        ...
        self.scattered_structs_dropdown = ttk.Combobox(
            self.treasure_trove_cove_frame,
            textvariable=self.scattered_structs_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
            width=15,
        ...
        self.scattered_structs_dropdown.grid(
            row=0, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.super_slippery_ttc_ttp_canvas = tk.Label(
            self.treasure_trove_cove_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.super_slippery_ttc_ttp_canvas.grid(
            row=1, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.super_slippery_ttc_checkbox_ttp = self.CreateToolTip(
            self.super_slippery_ttc_ttp_canvas,
            self,
            tool_tips_dict["TREASURE_TROVE_COVE"]["SUPER_SLIPPERY_SAND"],
        )
        self.super_slippery_ttc_var = tk.IntVar()
        self.super_slippery_ttc_checkbox = tk.Checkbutton(
            self.treasure_trove_cove_frame,
            text="Super Slippery Sand",
            variable=self.super_slippery_ttc_var,
            selectcolor=curr_background_color,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.super_slippery_ttc_checkbox.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        # Clanker's Cavern
        ...
        self.clankers_cavern_frame = tk.LabelFrame(
            self._clankers_cavern_tab,
            text="Clanker's Cavern",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        ...
        self.hard_rings_ttp_canvas = tk.Label(
            self.clankers_cavern_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.hard_rings_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.hard_rings_checkbox_ttp = self.CreateToolTip(
            self.hard_rings_ttp_canvas,
            self,
            tool_tips_dict["CLANKERS_CAVERN"]["SHUFFLE_CLANKER_RING_ORDER"],
        )
        self.hard_rings_var = tk.IntVar()
        self.hard_rings_checkbox = tk.Checkbutton(
            self.clankers_cavern_frame,
            text="Shuffle Clanker Ring Order",
            variable=self.hard_rings_var,
            selectcolor=curr_background_color,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.hard_rings_checkbox.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        # Bubblegloop Swamp
        ...
        self.bubblegloop_swamp_frame = tk.LabelFrame(
            self._bubblegloop_swamp_tab,
            text="Bubblegloop Swamp",
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        ...
        self.croctus_ttp_canvas = tk.Label(
            self.bubblegloop_swamp_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.croctus_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.croctus_checkbox_ttp = self.CreateToolTip(
            self.croctus_ttp_canvas,
            self,
            tool_tips_dict["BUBBLEGLOOP_SWAMP"]["SHUFFLE_CROCTUS_ORDER"],
        )
        self.croctus_var = tk.IntVar()
        self.croctus_checkbox = tk.Checkbutton(
            self.bubblegloop_swamp_frame,
            text="Shuffle Croctus Order",
            variable=self.croctus_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.croctus_checkbox.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.mr_vile_ttp_canvas = tk.Label(
            self.bubblegloop_swamp_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.mr_vile_ttp_canvas.grid(
            row=1, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.mr_vile_checkbox_ttp = self.CreateToolTip(
            self.mr_vile_ttp_canvas,
            self,
            tool_tips_dict["BUBBLEGLOOP_SWAMP"]["MR_VILE_BIGGER_BADDER_CROCODILE"],
        )
        self.mr_vile_var = tk.IntVar()
        self.mr_vile_checkbox = tk.Checkbutton(
            self.bubblegloop_swamp_frame,
            text="Mr. Vile: Bigger, Badder Crocodile",
            variable=self.mr_vile_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.mr_vile_checkbox.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.tiptup_choir_ttp_canvas = tk.Label(
            self.bubblegloop_swamp_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.tiptup_choir_ttp_canvas.grid(
            row=2, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.tiptup_choir_checkbox_ttp = self.CreateToolTip(
            self.tiptup_choir_ttp_canvas,
            self,
            tool_tips_dict["BUBBLEGLOOP_SWAMP"]["TIPTUP_CHOIR_NO_ASSIGNED_SEATS"],
        )
        self.tiptup_choir_var = tk.IntVar()
        self.tiptup_choir_checkbox = tk.Checkbutton(
            self.bubblegloop_swamp_frame,
            text="Tiptup Choir: No Assigned Seats",
            variable=self.tiptup_choir_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.tiptup_choir_checkbox.grid(
            row=2, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        # Freezeezy Peak
        ...
        self.freezeezy_peak_frame = tk.LabelFrame(
            self._freezeezy_peak_tab,
            text="Freezeezy Peak",
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        ...
        self.hard_races_ttp_canvas = tk.Label(
            self.freezeezy_peak_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.hard_races_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.hard_races_checkbox_ttp = self.CreateToolTip(
            self.hard_races_ttp_canvas,
            self,
            tool_tips_dict["FREEZEEZY_PEAK"]["BOGGY_RACES_MOVED_FLAGS"],
        )
        self.hard_races_var = tk.IntVar()
        self.hard_races_checkbox = tk.Checkbutton(
            self.freezeezy_peak_frame,
            text="Boggy Races: Moved Flags",
            variable=self.hard_races_var,
            selectcolor=curr_background_color,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.hard_races_checkbox.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        # Gobi's Valley
        ...
        self.gobis_valley_frame = tk.LabelFrame(
            self._gobis_valley_tab,
            text="Gobi's Valley",
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        ...
        self.ancient_ones_ttp_canvas = tk.Label(
            self.gobis_valley_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.ancient_ones_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.ancient_ones_checkbox_ttp = self.CreateToolTip(
            self.ancient_ones_ttp_canvas,
            self,
            tool_tips_dict["GOBIS_VALLEY"]["SHUFFLED_ANCIENT_ONES_ORDER"],
        )
        self.ancient_ones_var = tk.IntVar()
        self.ancient_ones_checkbox = tk.Checkbutton(
            self.gobis_valley_frame,
            text="Shuffle Ancient Ones Order",
            variable=self.ancient_ones_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.ancient_ones_checkbox.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.maze_jinxy_heads_ttp_canvas = tk.Label(
            self.gobis_valley_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.maze_jinxy_heads_ttp_canvas.grid(
            row=1, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.maze_jinxy_heads_checkbox_ttp = self.CreateToolTip(
            self.maze_jinxy_heads_ttp_canvas,
            self,
            tool_tips_dict["GOBIS_VALLEY"]["SHUFFLE_MAZE_JINXY_HEADS_ORDER"],
        )
        self.maze_jinxy_heads_var = tk.IntVar()
        self.maze_jinxy_heads_checkbox = tk.Checkbutton(
            self.gobis_valley_frame,
            text="Shuffle Maze Jinxy Heads Order",
            variable=self.maze_jinxy_heads_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.maze_jinxy_heads_checkbox.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.matching_puzzle_ttp_canvas = tk.Label(
            self.gobis_valley_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.matching_puzzle_ttp_canvas.grid(
            row=2, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.matching_puzzle_checkbox_ttp = self.CreateToolTip(
            self.matching_puzzle_ttp_canvas,
            self,
            "Randomized Matching Puzzle Not Implemented Yet",
        )
        self.matching_puzzle_var = tk.IntVar()
        self.matching_puzzle_checkbox = tk.Checkbutton(
            self.gobis_valley_frame,
            text="Randomize Matching Puzzle",
            variable=self.matching_puzzle_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.matching_puzzle_checkbox.grid(
            row=2, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        # Mad Monster Mansion
        ...
        self.mad_monster_mansion_frame = tk.LabelFrame(
            self._mad_monster_mansion_tab,
        ...
        self.lit_pots_ttp_canvas = tk.Label(
            self.mad_monster_mansion_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.lit_pots_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.lit_pots_checkbox_ttp = self.CreateToolTip(
            self.lit_pots_ttp_canvas,
            self,
            tool_tips_dict["MAD_MONSTER_MANSION"]["POTS_ARE_LIT"],
        )
        self.lit_pots_var = tk.IntVar()
        self.lit_pots_checkbox = tk.Checkbutton(
            self.mad_monster_mansion_frame,
            text="Pots Are Lit",
            variable=self.lit_pots_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.lit_pots_checkbox.grid(
            row=0, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.lit_pots_ttp_canvas = tk.Label(
            self.mad_monster_mansion_frame,
        ...
        self.motzand_keys_checkbox_ttp = self.CreateToolTip(
            self.lit_pots_ttp_canvas,
            self,
            tool_tips_dict["MAD_MONSTER_MANSION"]["MOTZAND_KEYS"],
        )
        self.motzand_keys_var = tk.IntVar()
        self.motzand_keys_checkbox = tk.Checkbutton(
            self.mad_monster_mansion_frame,
            text="Randomize Motzand's Song",
            variable=self.motzand_keys_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.motzand_keys_checkbox.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        # Rusty Bucket Bay
        ...
        self.rusty_bucket_bay_frame = tk.LabelFrame(
            self._rusty_bucket_bay_tab,
            text="Rusty Bucket Bay",
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        ...
        self.buttons_ttp_canvas = tk.Label(
            self.rusty_bucket_bay_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.buttons_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.buttons_checkbox_ttp = self.CreateToolTip(
            self.buttons_ttp_canvas,
            self,
            tool_tips_dict["RUSTY_BUCKET_BAY"]["RANDOMIZED_BUTTON_COMBO"],
        )
        self.buttons_var = tk.IntVar()
        self.buttons_checkbox = tk.Checkbutton(
            self.rusty_bucket_bay_frame,
            text="Randomized Button Combo",
        ...
        self.click_clock_wood_frame = tk.LabelFrame(
            self._click_clock_wood_tab,
            text="Click Clock Wood",
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.large_font_size),
        )
        ...
        self.ccw_ttp_canvas = tk.Label(
            self.click_clock_wood_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.ccw_ttp_canvas.grid(
            row=0, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.ccw_checkbox_ttp = self.CreateToolTip(
            self.ccw_ttp_canvas, self, tool_tips_dict["CLICK_CLOCK_WOOD"]["SHUFFLE_BY"]
        )
        self.ccw_text = tk.Label(
            self.click_clock_wood_frame,
            text="Shuffle By:",
            foreground=self.white,
            background=curr_background_color,
        ...
        self.ccw_dropdown = ttk.Combobox(
            self.click_clock_wood_frame,
            textvariable=self.ccw_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        ...
        self.ccw_dropdown.grid(
            row=0, column=2, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.ccw_open_var = tk.IntVar()
        self.open_click_clock_wood_checkbox = tk.Checkbutton(
            self.click_clock_wood_frame,
            text="Open Seasons",
            variable=self.ccw_open_var,
            selectcolor=curr_background_color,
            foreground=self.white,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.open_click_clock_wood_checkbox.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        #########################################
        ### END OF WORLD SPECIFIC TAB CONTROL ###
        ...
        # Remove Files
        self.remove_files_ttp_canvas = tk.Label(
            self.misc_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.remove_files_ttp_canvas.grid(
            row=1, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.remove_files_checkbutton_ttp = self.CreateToolTip(
            self.remove_files_ttp_canvas,
            self,
            tool_tips_dict["MISC_OPTIONS"]["REMOVE_EXTRA_FILES"],
        )
        self.remove_files_var = tk.IntVar()
        self.remove_files_checkbutton = tk.Checkbutton(
            self.misc_frame,
            text="Remove Extra Files",
            variable=self.remove_files_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.remove_files_checkbutton.grid(
            row=1, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        # Tool Tips
        self.tool_tips_ttp_canvas = tk.Label(
            self.misc_frame,
            image=self.ttp_image,
            foreground=self.black,
            background=curr_background_color,
        ...
        self.tool_tips_ttp_canvas.grid(
            row=2, column=0, padx=self.padx, pady=self.pady, sticky="w"
        )
        self.tool_tips_checkbutton_ttp = self.CreateToolTip(
            self.tool_tips_ttp_canvas, self, tool_tips_dict["MISC_OPTIONS"]["TOOL_TIPS"]
        )
        self.tool_tips_var = tk.IntVar()
        self.tool_tips_checkbutton = tk.Checkbutton(
            self.misc_frame,
            text="Show Tool Tips",
            variable=self.tool_tips_var,
            foreground=self.black,
            background=curr_background_color,
            font=(self.font_type, self.medium_font_size),
        )
        self.tool_tips_checkbutton.grid(
            row=2, column=1, padx=self.padx, pady=self.pady, sticky="w"
        )
        ##########################
        ### END OF TAB CONTROL ###
        ...
        # Load
        self._load_config_image = tk.PhotoImage(
            file=(self.cwd / "Pictures" / "Load_Config.png")
        )
        self.load_config_button = tk.Button(
            self.config_and_submit,
            text="Load Config",
            command=self._load_configuration,
            image=self._load_config_image,
            background=self.generic_background_color,
        )
        self.load_config_button.pack(expand=tk.TRUE, fill=tk.BOTH, side="left")
        # Save
        self._save_config_image = tk.PhotoImage(
            file=(self.cwd / "Pictures" / "Save_Config.png")
        )
        self.save_config_button = tk.Button(
            self.config_and_submit,
            text="Save Config",
            command=self._save_current_configuration,
            image=self._save_config_image,
            background=self.generic_background_color,
        )
        self.save_config_button.pack(expand=tk.TRUE, fill=tk.BOTH, side="left")
        # Read Me
        self._read_me_image = tk.PhotoImage(
            file=(self.cwd / "Pictures" / "Read_Me.png")
        )
        self.read_me = tk.Button(
            self.config_and_submit,
            text="Open ReadMe",
            command=(lambda: self._open_file(self.cwd / "ReadMe.txt")),
            image=self._read_me_image,
            background=self.generic_background_color,
        )
        self.read_me.pack(expand=tk.TRUE, fill=tk.BOTH, side="left")
        # Submit
        self._submit_image = tk.PhotoImage(file=(self.cwd / "Pictures" / "Submit.png"))
        self.submit = tk.Button(
            self.config_and_submit,
            text="Submit",
            command=self._submit,
            image=self._submit_image,
            background=self.generic_background_color,
        )
