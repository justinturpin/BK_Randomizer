...
class Speech_File_Class:
    def __init__(self, paths: BkRandoPaths, pointer: str) -> None:
        self.paths = paths
        with self.paths.decompressed_path(pointer).open("r+b") as decomp_file:
    ...
    def _breakdown(self):
        ...
        while curr_index < mm_len:
            ...
            # This Is An Empty String
                ...
                empty_string = (
                    leading_zeros(str(hex(first_index))[2:], 2).upper()
            ...
            # This Is A Selectable Option (Furnace Fun)
                ...
                for text_index in range(curr_index + 3, curr_index + 2 + third_index):
                    curr_text += leading_zeros(
                        str(hex(self.mm_decomp[text_index]))[2:], 2
                    ).upper()
            ...
            if len(curr_text) > 0:
                if readable and isinstance(curr_sprite, int):
                    ...
                    for char_index in range(0, len(curr_text), 2):
                        readable_text += bytearray.fromhex(
                            curr_text[char_index : char_index + 2]
                        ).decode()
    ...
    def _furnace_fun_question_format(
        self, question_part1, question_part2, answer1, answer2, answer3
    ):
        self.mm_decomp.resize(
            27
            + len(question_part1)
            + len(question_part2)
            + len(answer1)
