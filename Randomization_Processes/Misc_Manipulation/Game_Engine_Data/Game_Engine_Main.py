...
class Game_Engine_Class:
    ...
    def __init__(self, paths: BkRandoPaths):
        self.paths = paths
        # self._patch_file()
        with self.paths.decompressed_path("F37F90").open("r+b") as decomp_file:
            self.mm = mmap.mmap(decomp_file.fileno(), 0)

    def _starting_moves(self) -> None:
        """
    ...
    def _mumbo_transformations_costs(
        self,
        termite_cost=0,
        crocodile_cost=0,
        walrus_cost=0,
    ...
    def _starting_lives(self, life_count) -> None:
    ...
    def _open_df(self, offset: str) -> io.IOBase:
        # return open(f"{self._file_dir}Randomized_ROM/{offset}-Decompressed.bin", "r+b")
        return self.paths.decompressed_path(offset).open("r+b")
    ...
    def _patch_ttc_yumyum_crashfix(self):
        ...
        with self._open_df(ovl_core1_text) as f:
            ...
            m.write(
                (
                    0x1100000D000000008D0A0000000A4E0234010080552100083C098000012A4826
                ).to_bytes(32, "big")
            )
            m.write(
                (
                    0x3C0100400121482A1120000300000000080E1C2200000000080E1C2800000000
                ).to_bytes(32, "big")
            )

    def _patch_antiantitamper(
        self,
        core1_at=True,
        core2_at=True,
        sm_at=True,
