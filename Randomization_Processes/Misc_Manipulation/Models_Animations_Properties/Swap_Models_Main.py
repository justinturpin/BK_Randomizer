...
class Swap_Models_Manipulation_Class:
    ...
    def __init__(self, seed_val: int, paths: BkRandoPaths, model_dict: dict) -> None:
        self._paths = paths
    ...
    def _grab_compressed_file(self, pointer_str: str) -> None:
        ...
        with self._paths.compressed_path(file_name).open("w+b") as comp_file:
    ...
    def _place_compressed_files(self) -> None:
        ...
        for pointer in range(self._pointers_start, self._pointers_end + 0x08, 0x08):
            ...
            pointer_content = curr_pointer_path.read_bytes()
    ...
    def _shuffle_list(self, original_list) -> None:
        """
    ...
    def _clean_up(self) -> None:
        for pointer in range(self._pointers_start, self._pointers_end + 0x08, 0x08):
            ...
            pointer_str = str(hex(pointer))
            curr_pointer_file = self._model_address_dict[pointer_str]
            file_path = self._paths.randomized_rom_dir / curr_pointer_file
            if file_path.is_file():
                file_path.unlink()

    def _model_manip_main(self) -> None:
        """
        ...
        for pointer in range(self._pointers_start, self._pointers_end + 8, 8):
            ...
            self._model_address_dict[pointer_str.lower()] = (
        ...
        for category in self._model_dict:
            if "Replacements" in self._model_dict[category]:
                ...
                for original_file in self._model_dict[category]["Original"]:
                    ...
                    self._model_address_dict[original_file.lower()] = (
                        f"{replacement_model[2:].lower()}-Compressed.bin"
                    )
                ...
                for curr_set in self._model_dict[category]:
                    ...
                    for model in self._model_dict[category][curr_set]:
                        swap_lists[set_counter].append(
                            f"{model[2:].lower()}-Compressed.bin"
                        )
                ...
                for curr_set in self._model_dict[category]:
                    ...
                    for model in self._model_dict[category][curr_set]:
                        if set_count == (set_counter + 1):
                            self._model_address_dict[model.lower()] = swap_lists[0][
                                model_count
                            ]
                        else:
                            self._model_address_dict[model.lower()] = swap_lists[
                                set_counter + 1
                            ][model_count]
                        model_count += 1
                    set_counter += 1
