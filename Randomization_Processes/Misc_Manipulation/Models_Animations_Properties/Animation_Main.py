...
class Swap_Animations_Manipulation:
    ...
    def __init__(
        self, seed_val: int, paths: BkRandoPaths, animations_dict: dict
    ) -> None:
        self._paths = paths
        self._file_bytes = paths.rom_path.read_bytes()
        self._seed_val = seed_val
        self._pointers_start = 0
        self._pointers_end = 0
        self._animation_address_dict = {}
        self._animation_dict = animations_dict

    def _grab_compressed_file(self, pointer_str: str) -> None:
        """
        ...
        file_name = f"{pointer_str[2:].lower()}"
        compressed_path = self._paths.compressed_path(file_name)
        with compressed_path.open("w+b") as comp_file:
    ...
    def _place_compressed_files(self) -> None:
        ...
        """
        if self._pointers_start is None:
            raise ValueError("No start pointers found!")

        if self._pointers_end is None:
            raise ValueError("No end pointers found!")
        ...
        for pointer in range(self._pointers_start, self._pointers_end + 0x08, 0x08):
            ...
            curr_pointer_path = self._paths.randomized_rom_dir / curr_pointer_file
            with curr_pointer_path.open("r+b") as comp_file:
            ...
            with self._paths.rom_path.open("r+b") as rom_file:
                ...
                if pointer < self._pointers_end:
                    address_end = (
                        address_start + len(pointer_content) - int("0x10CD0", 16)
                    )
    ...
    def _shuffle_list(self, original_list: list[T]) -> list[T]:
        """
    ...
    def _animation_manip_main(self) -> None:
        """
        ...
        for pointer in range(self._pointers_start, self._pointers_end + 8, 8):
            ...
            self._animation_address_dict[pointer_str.lower()] = (
                f"{pointer_str[2:].lower()}-Compressed.bin"
            )
        for category in self._animation_dict:
            if "Replacements" in self._animation_dict[category]:
                ...
                for original_file in self._animation_dict[category]["Original"]:
                    ...
                    self._animation_address_dict[original_file.lower()] = (
                        f"{replacement_animation[2:].lower()}-Compressed.bin"
                    )
                ...
                for curr_set in self._animation_dict[category]:
                    ...
                    for animation in self._animation_dict[category][curr_set]:
                        swap_lists[set_counter].append(
                            f"{animation[2:].lower()}-Compressed.bin"
                        )
                    swap_lists[set_counter] = self._shuffle_list(
                        swap_lists[set_counter]
                    )
                    set_counter += 1
                # Assign New Files
                ...
                for curr_set in self._animation_dict[category]:
                    ...
                    for animation in self._animation_dict[category][curr_set]:
                        if set_count == (set_counter + 1):
                            self._animation_address_dict[animation.lower()] = (
                                swap_lists[0][animation_count]
                            )
                        else:
                            self._animation_address_dict[animation.lower()] = (
                                swap_lists[set_counter + 1][animation_count]
                            )
                        animation_count += 1
                    set_counter += 1
