...
class SetupFile:
    ...
    def __init__(self, setup_address, paths: BkRandoPaths, setup_name=None) -> None:
        """
        ...
        with self.paths.decompressed_path(setup_address).open("r+b") as f:
    ...
    def _locate_item_index(self, item_search_string, item_type=None, seed=None) -> None:
        """
    ...
    def _obtain_struct_parameters(self, item_list) -> None:
    ...
    def _set_struct(self, struct_index, struct_info) -> None:
        """
    ...
    def _obtain_object_parameters(self, item_list, item_type) -> None:
        """
    ...
    def _set_object(self, object_index, object_info) -> None:
        ...
        if (
            (object_info["Script1"] == 0x19)
            and (object_info["Script2"] == 0x0C)
            and (object_info["Obj_ID1"] == 0x02)
            and (object_info["Obj_ID2"] == 0x89)
        ):
    ...
    def _locate_flagged_object_index(
        self, object_search_string, flag_search_string
    ) -> None:
        """
    ...
    def _obtain_flagged_object_parameters(self, object_index, flag_index) -> None:
        """
    ...
    def _obtain_sequence_tuple_parameters(self, item_index1, item_index2) -> None:
        """
    ...
    def _set_sequence_tuple(
        self, item_index1, item_info1, item_index2, item_info2
    ) -> None:
    ...
    def _locate_camera_sequence_index(
        self, object_search_string, camera_search_string
    ) -> None:
        """
        Finds the index of every sequence item with its associated camera, then run the
        functions to pull their information
        """
        object_index = self.mm.find(bytes.fromhex(object_search_string))
    ...
    def _obtain_camera_sequence_parameters(self, sequence_index, camera_index) -> None:
    ...
    def _set_camera_sequence(
        self, sequence_index, sequence_info, camera_index, camera_info
    ):
    ...
    def _replace_each_object_parameters(
        self, search_string_list, replacement_list
    ) -> None:
        """
        ...
        for index_num in range(len(search_string_list)):
            self._edit_object(
                search_string_list[index_num], replacement_list[index_num]
            )

    def _replace_all_in_area(self, search_string, replace_list) -> None:
        """
    ...
    def _remove_object(self, item_list) -> None:
        """
    ...
    def _edit_object(self, item_search_string, replacement_dict) -> None:
    ...
    def _edit_object_index(self, item_index_start, replacement_dict) -> None:
        """Takes specific parameters of an object and replaces them with the info from a dictionary"""
        for index_add in replacement_dict:
            self.mm[item_index_start + index_add] = replacement_dict[index_add]

    def _does_string_exist(self, item_search_string) -> bool:
        """
    ...
    def adjust_ttc_oob_egg(self) -> None:
    ...
    def adjust_ttc_lighthouse_token(self) -> None:
        """Unused"""
        pass

    def _skip_non_ring(self, item_index) -> bool:
    ...
    def _obtain_value_at_index(self, index_start, index_length):
        ...
        for index_add in range(index_length):
            value += self.mm[index_start + index_add] << (
                (index_length - index_add - 1) * 8
            )
        return value
