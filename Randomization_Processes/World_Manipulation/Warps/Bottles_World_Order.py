...
class World_Order_Bottles:
    def __init__(
        ...
        seed_val=0,
        max_hp="One Health Only",
        final_puzzle_option=0,
        world_exit_option="Exit From World You Were Just In",
        removed_detransformations=0,
        free_transformations="Base Game Costs",
    ):
    ...
    def _progression_requirements(self, world_name):
        ...
        # Exiting TTC -> Going To CC
            ...
            if self.world_exit_option == "Exit From World You Were Just In":
                if (world_name != "Clanker's Cavern") and (
                    "Clanker's Cavern" not in self.world_order_list
                ):
        ...
        # Exiting CC -> Going To BGS
            ...
            if self.world_exit_option == "Exit From World You Were Just In":
                if (world_name == "Bubblegloop Swamp") or (
                    "Bubblegloop Swamp" in self.world_order_list
                ):
        ...
        # Exiting BGS -> Going To FP
            ...
            if self.world_exit_option == "Exit From World You Were Just In":
                if (world_name == "Freezeezy Peak") or (
                    "Freezeezy Peak" in self.world_order_list
                ):
                    # You Can Enter World If Leaving It
                    required_move_list = []
                elif world_name not in [
                    "Gobi's Valley",
                    "Mad Monster Mansion",
                    "Rusty Bucket Bay",
                    "Click Clock Wood",
        ...
        # Exiting FP -> Going To GV
            ...
            if self.world_exit_option == "Exit From World You Were Just In":
                if (world_name != "Gobi's Valley") and (
                    "Gobi's Valley" not in self.world_order_list
                ):
    ...
    def _possible_world_moves(
        self, world_name, progress_move_list, additional_learned_moves={}
    ):
        ...
        for possible_bottles in self.bottles_world_warp_dict[world_name][
            "Possible_Bottles"
        ]:
            for requirement_list in self.bottles_world_warp_dict[world_name][
                "Possible_Bottles"
            ][possible_bottles]:
                required_moves = []
                for requirement in requirement_list:
                    if (requirement not in self.learned_moves) and (
                        requirement not in additional_learned_moves
                    ):
                        required_moves.append(requirement)
                if (len(required_moves) == 0) and (
                    possible_bottles not in additional_learned_moves
                ):
        ...
        if len(progress_move_list) <= len(available_bottles):
            for progress_move in progress_move_list:
                ...
                self.temp_learned_moves[world_name]["New_Moves_List"].append(
                    progress_move
                )
                available_bottles.remove(selected_bottles)
            learnable_moves = []
            for move in self.remaining_moves:
                if (move not in list(additional_learned_moves.values())) and (
                    move not in self.temp_learned_moves[world_name]["New_Moves_List"]
                ):
                    learnable_moves.append(move)
            while (
                (len(self.temp_learned_moves[world_name]["New_Moves"]) < 2)
                and (len(available_bottles) > 0)
                and (len(learnable_moves) > 0)
            ):
                seed(a=(self.seed_val + self.increment))
                self.increment += 1
        ...
        # If not, can you make more spots to progress?
            ...
            selected_move = max(
                set(required_moves_for_additional_slot),
                key=required_moves_for_additional_slot.count,
            )
            seed(a=(self.seed_val + self.increment))
            self.increment += 1
            selected_bottles = choice(available_bottles)
            ...
            self._possible_world_moves(
                world_name,
                progress_move_list,
                additional_learned_moves=self.temp_learned_moves[world_name][
                    "New_Moves"
                ],
            )
        else:
        ...
        for progress_move in progress_move_list:
            if (
                progress_move
                not in self.temp_learned_moves[world_name]["New_Moves_List"]
            ):
                return False
        return True
    ...
    def _possible_world_transformation(self, world_name):
        ...
        for flagged_object_flag in self.bottles_world_warp_dict[world_name][
            "Flagged_Object_Flags"
        ]:
            object_type = self.bottles_world_warp_dict[world_name][
                "Flagged_Object_Flags"
            ][flagged_object_flag]["Type"]
            object_id = self.bottles_world_warp_dict[world_name][
                "Flagged_Object_Flags"
            ][flagged_object_flag]["ID"]
            if (
                (object_type == "Mumbo Token")
                and (object_id not in self.collected_mumbo_token_list)
                and (
                    object_id
                    not in self.temp_learned_moves[world_name]["New_Mumbo_Tokens"]
                )
            ):
                for requirement_list in self.bottles_world_warp_dict[world_name][
                    "Flagged_Object_Flags"
                ][flagged_object_flag]["Requirements"]:
                    can_obtain = True
                    for requirement in requirement_list:
                        if (requirement not in self.learned_moves) and (
                            requirement
                            ...
                            if (
                                requirement
                                in ["Termite", "Crocodile", "Walrus", "Pumpkin", "Bee"]
                            ) and (len(requirement_list) == 1):
                                transformation_tokens.append(object_id)
                            can_obtain = False
                            break
                    if can_obtain:
                        self.temp_learned_moves[world_name]["New_Mumbo_Tokens"].append(
                            object_id
                        )
        # Is there a transformation here and can you afford it?
        current_mumbo_count = len(set(self.collected_mumbo_token_list)) + len(
            set(self.temp_learned_moves[world_name]["New_Mumbo_Tokens"])
        )
        ...
        if world_name == "Mumbo's Mountain":
            if ("Termite" not in self.learned_moves) and (
                current_mumbo_count >= next_transform_cost
            ):
                self.temp_learned_moves[world_name]["New_Moves_List"].append("Termite")
        elif world_name == "Bubblegloop Swamp":
            if ("Crocodile" not in self.learned_moves) and (
                current_mumbo_count >= next_transform_cost
            ):
                self.temp_learned_moves[world_name]["New_Moves_List"].append(
                    "Crocodile"
    ...
    def _possible_world_jiggies(self, world_name):
        ...
        for flagged_object_flag in self.bottles_world_warp_dict[world_name][
            ...
            if (
                (object_type == "Jiggy")
                and (object_id not in self.collected_jiggy_list)
                and (
                ...
                for requirement_list in self.bottles_world_warp_dict[world_name][
                    ...
                    for requirement in requirement_list:
                        if (requirement not in self.learned_moves) and (
                            ...
                            break
                    if can_obtain:
                        self.temp_learned_moves[world_name]["New_Jiggies"].append(
                            object_id
                        )
    ...
    def _possible_lair_collectables(self, world_name):
        ...
        for area_name in self.extra_flagged_object_flags:
            ...
            for object_flag in self.extra_flagged_object_flags[area_name]:
                object_type = self.extra_flagged_object_flags[area_name][object_flag][
                    "Type"
                ]
                object_id = self.extra_flagged_object_flags[area_name][object_flag][
                    "ID"
                ]
                if (
                    (object_type == "Jiggy")
                    ...
                    and (
                        object_id
                        not in self.temp_learned_moves[world_name]["New_Jiggies"]
                    )
                    and (
                        object_id
                        not in self.temp_learned_moves[area_name]["New_Jiggies"]
                    )
                ):
                    for requirement_list in self.extra_flagged_object_flags[area_name][
                        object_flag
                    ]["Requirements"]:
                        can_obtain = True
                        for requirement in requirement_list:
                            if (requirement not in self.learned_moves) and (
                                requirement
                                not in self.temp_learned_moves[world_name][
                                    "New_Moves_List"
                                ]
                            ):
                                if (
                                ...
                                can_obtain = False
                                break
                        if (self.world_exit_option == 1) and (
                            (len(self.world_order_list) + 1)
                            < self.extra_flagged_object_flags[area_name][object_flag][
                                "World_Count"
                            ]
                        ):
                            can_obtain = False
                        if can_obtain:
                            self.temp_learned_moves[area_name]["New_Jiggies"].append(
                                object_id
                            )
                if (
                    (object_type == "Mumbo Token")
                    ...
                    and (
                        object_id
                        not in self.temp_learned_moves[area_name]["New_Mumbo_Tokens"]
                    )
                ):
                    for requirement_list in self.extra_flagged_object_flags[area_name][
                        object_flag
    ...
    def _possible_next_worlds(self):
        ...
        for world_name in self.remaining_worlds:
            ...
            self.temp_learned_moves[world_name]["New_Mumbo_Tokens"] = []
            self.temp_learned_moves[world_name]["New_Jiggies"] = []
            required_jiggy_count, progress_move_list = self._progression_requirements(
                world_name
            )
            move_progressable = self._possible_world_moves(
                world_name, progress_move_list
            )
            if move_progressable:
                ...
                for past_world_name in self.world_order_list:
                    for jiggy_id in self.temp_learned_moves[past_world_name][
                        "New_Jiggies"
                    ]:
                        possible_total_jiggy_list.append(jiggy_id)
                for area_name in self.extra_flagged_object_flags:
    ...
    def _set_next_world(self, next_world):
        ...
        for bottles_location in self.temp_learned_moves[next_world]["New_Moves"]:
            self.world_order_dict[next_world]["Learned_Moves"][bottles_location] = (
                self.temp_learned_moves[next_world]["New_Moves"][bottles_location]
            )
        ...
        if (transformation_name) and (
            transformation_name not in self.transform_cost_dict
        ):
            if self.free_transformations == "Free Transformations":
                ...
                self.transform_cost_dict[transformation_name] = (
                    len(self.transform_cost_dict) + 1
                ) * 5
        ...
        for remaining_move in self.remaining_moves:
            ...
            for world_name in self.world_order_list:
                for possible_bottles in self.bottles_world_warp_dict[world_name][
                    ...
                    if (
                        possible_bottles
                        not in self.world_order_dict[world_name]["Learned_Moves"]
                    ):
                        for requirement_list in self.bottles_world_warp_dict[
                            world_name
                        ]["Possible_Bottles"][possible_bottles]:
                            required_moves = []
                            for requirement in requirement_list:
            ...
            self.world_order_dict[available_bottles[selected_bottles]]["Learned_Moves"][
                selected_bottles
            ] = remaining_move
            ...
            while self.remaining_worlds:
                ...
                if self.world_exit_option == "Exit From Entrance You Entered From":
                    if self.removed_detransformations == 0:
                        ...
                        if (len(self.world_order_list) == 6) and (
                            "Mad Monster Mansion" not in self.world_order_list
                        ):
                            if "Mad Monster Mansion" in possible_world_list:
                                next_world = "Mad Monster Mansion"
                            else:
                                self._restart(reason="No Mad Monster Mansion 2")
                        elif (len(self.world_order_list) < 4) and (
                            "Mad Monster Mansion" in possible_world_list
                        ):
                            possible_world_list.remove("Mad Monster Mansion")
