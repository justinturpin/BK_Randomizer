...
from Randomization_Processes.Misc_Manipulation.Misc_Manipulation_Main import (
    Misc_Manipulation_Class,
)
from Randomization_Processes.Setup_Functions import (
    make_copy_of_rom,
    set_seed,
    setup_tmp_folder,
)
from Randomization_Processes.World_Manipulation.World_Manipulation_Main import (
    World_Manipulation_Class,
)
...
class Progression_GUI_Class:
    ...
    def __init__(self, master: "User_GUI_Class") -> None:
        self.master = master
        ...
        self.progress_bar_window.winfo_toplevel().title(
            "Banjo-Kazooie Randomizer Progress Bar"
        )
        #         self.progress_bar_window.geometry("400x50")
        self.progress_bar_window.config(background="#F3E5AB")
        self.progress_bar_value = 0
        self._mumbo_error_message = (
            "Please Check The ReadMe Under 'GUI Errors/Warnings' For Instructions"
        )
    ...
    class App_Variable_Label:
        ...
        def set_text(self, new_text: str) -> None:
            """
            Changes the text to new_text
            """
            self.text.set(new_text)

        def pack_label(self) -> None:
            """
            Displays label
            """
            self.label.pack()
    ...
    class App_ProgressBar:
        ...
        def __init__(self, window, bar_length=390, bar_mode="determinate"):
            self.progressbar = ttk.Progressbar(
                window,
                style="bk.Horizontal.TProgressbar",
                orient=HORIZONTAL,
                length=bar_length,
                mode=bar_mode,
            )
    ...
    def _hidden_random_values(self) -> None:
        """
        ...
        if self.master.final_note_door_value.get() == "?":
            ...
            self.master.final_note_door_val = int(
                self.master.final_note_door_value.get()
            )
        self.master.logger.debug(
            f"Final Note Door Val: {self.master.final_note_door_val}"
        )
        # Final Puzzle
        ...
        if self.master.before_blue_egg_carry_value.get() == "?":
            ...
            self.master.before_blue_egg = int(
                self.master.before_blue_egg_carry_value.get()
            )
        self.master.logger.debug(
            f"Before Cheato Blue Egg: {self.master.before_blue_egg}"
        )
        if self.master.after_blue_egg_carry_value.get() == "?":
            ...
            self.master.after_blue_egg = int(
                self.master.after_blue_egg_carry_value.get()
            )
        ...
        if self.master.before_red_feather_carry_value.get() == "?":
            ...
            cheat_sheet += (
                f"Before Cheato Red Feather: {self.master.before_red_feather}\n"
            )
        else:
            self.master.before_red_feather = int(
                self.master.before_red_feather_carry_value.get()
            )
        self.master.logger.debug(
            f"Before Cheato Red Feather: {self.master.before_red_feather}"
        )
        if self.master.after_red_feather_carry_value.get() == "?":
            ...
            cheat_sheet += (
                f"After Cheato Red Feather: {self.master.after_red_feather}\n"
            )
        else:
            self.master.after_red_feather = int(
                self.master.after_red_feather_carry_value.get()
            )
        self.master.logger.debug(
            f"After Cheato Red Feather: {self.master.after_red_feather}"
        )
        ...
        if self.master.before_gold_feather_carry_value.get() == "?":
            ...
            cheat_sheet += (
                f"Before Cheato Gold Feather: {self.master.before_gold_feather}\n"
            )
        else:
            self.master.before_gold_feather = int(
                self.master.before_gold_feather_carry_value.get()
            )
        self.master.logger.debug(
            f"Before Cheato Gold Feather: {self.master.before_gold_feather}"
        )
        if self.master.after_gold_feather_carry_value.get() == "?":
            ...
            self.master.after_gold_feather = randint(
                self.master.before_gold_feather, 255
            )
            cheat_sheet += (
                f"After Cheato Gold Feather: {self.master.after_gold_feather}\n"
            )
        else:
            self.master.after_gold_feather = int(
                self.master.after_gold_feather_carry_value.get()
            )
        self.master.logger.debug(
            f"After Cheato Gold Feather: {self.master.after_gold_feather}"
        )
        ...
        # Cheat Sheet
        cheat_sheet_path = (
            Path(self.master.cwd)
            / "Randomized_ROM"
            / f"HIDDEN_VALUES_CHEAT_SHEET_{self.seed_val}.txt"
        )
        cheat_sheet_path.write_text(cheat_sheet)
    ...
    def _setup(self) -> None:
        ...
        # Set the seed values that will be used for the randomization processes
        self.seed_val = set_seed(self.master.seed_value.get())
        self.master.paths.set_seed_val(self.seed_val)
        self._hidden_random_values()
        self.randomized_rom_path = make_copy_of_rom(self.master.paths, self.rom_path)
        gc.enable()

    def _decompress_main(self) -> None:
        """
        Runs the decompression functions
        """
        self.master.logger.info("Decompress Main")
        decompressor = Decompressor(self.master.paths)
    ...
    def _randomize_world(self) -> None:
        ...
        try:
            ...
            self.pb_label.set_text(
                f"Error Generating Worlds...\n{self._mumbo_error_message}"
            )
            raise
        # Specifically needs to go first
        if self.master.world_entrance_var.get() == "Bottles Shuffle":
            try:
                ...
                self.pb_label.set_text(
                    f"Error Preparing Bottles Shuffle...\n{self._mumbo_error_message}"
                )
                raise
        # Dropdown Boxes
        try:
            ...
            self.pb_label.set_text(
                f"Error Swapping Notes/Eggs/Feathers...\n{self._mumbo_error_message}"
            )
            raise
        try:
            ...
            self.pb_label.set_text(
                f"Error Moving Jinjos & Extra Lives...\n{self._mumbo_error_message}"
            )
            raise
        try:
        ...
        try:
            ...
            self.pb_label.set_text(
                f"Error Moving Jiggies/Tokens/Honeycombs...\n{self._mumbo_error_message}"
            )
            raise
        try:
        ...
        if self.master.remove_magic_barriers_var.get() == 1:
            try:
                ...
                self.pb_label.set_text(
                    f"Error Removing Magic Barriers...\n{self._mumbo_error_message}"
                )
                raise
        ...
        if self.master.hard_rings_var.get() == 1:
            try:
                ...
                self.pb_label.set_text(
                    f"Error Shuffling Clanker's Rings...\n{self._mumbo_error_message}"
                )
                raise
        ...
        if self.master.ancient_ones_var.get() == 1:
            try:
                ...
                self.pb_label.set_text(
                    f"Error Shuffling Ancient Ones...\n{self._mumbo_error_message}"
                )
                raise
        ...
        if self.master.skip_intro_cutscenes_var.get() == 1:
            try:
                ...
                self.pb_label.set_text(
                    f"Error Skip Intro Cutscenes...\n{self._mumbo_error_message}"
                )
        del world_manip
        self.master.logger.info("End of Randomize World")

    def _misc_options(self) -> None:
        ...
        try:
            ...
            self.pb_label.set_text(
                f"Error Making Manipulation Class...\n{self._mumbo_error_message}"
            )
            raise
        try:
            self.pb_label.set_text("Mumbo Make Bottles Say Silly Words...")
            misc_manip._bottles_requirements_text(
                self.master.final_note_door_var.get(),
                self.master.final_note_door_val,
                self.master.final_puzzle_var.get(),
                self.master.final_puzzle_val,
            )
        except Exception:
            self.warning_label.set_text("Uh-Oh...")
            self.pb_label.set_text(
                f"Error Adjusting Note & Jiggy Requirement...\n{self._mumbo_error_message}"
            )
            raise
        ...
        if self.master.motzand_keys_var.get() == 1:
            try:
                ...
                self.warning_label.set_text("Uh-Oh...")
                self.pb_label.set_text(
                    f"Error Randomizing Motzand's Song...\n{self._mumbo_error_message}"
                )
                raise
        if self.master.matching_puzzle_var.get() == 1:
            try:
                ...
                self.warning_label.set_text("Uh-Oh...")
                self.pb_label.set_text(
                    f"Error Shuffling Matching Puzzle...\n{self._mumbo_error_message}"
                )
                raise
        if self.master.bk_model_var.get() != "Default":
            try:
                self.pb_label.set_text(
                    "Stand On Skull And Press B To See Mighty Mumbo Magic..."
                )
                misc_manip._bk_model()
            except Exception:
                self.warning_label.set_text("Uh-Oh...")
                self.pb_label.set_text(
                    f"Error Adjusting BK's Colors...\n{self._mumbo_error_message}"
                )
                raise
        if self.master.jinjo_color_var.get() != "Default Jinjo Colors":
            try:
                ...
                self.warning_label.set_text("Uh-Oh...")
                self.pb_label.set_text(
                    f"Error Adjusting Jinjos' Colors...\n{self._mumbo_error_message}"
                )
                raise
        customization_option = False
        for custom_count, custom_name in enumerate(
            self.master.map_config_checkbox_dict
        ):
        ...
        try:
            ...
            misc_manip._shuffle_music(self.seed_val, self.master.paths)
        except Exception:
            self.pb_label.set_text(
                f"Error Shuffling Sounds/Jingles/Music...\n{self._mumbo_error_message}"
            )
            raise
        if customization_option:
            try:
                ...
                misc_manip._models_animations_properties(
                    self.seed_val, self.master.cwd, self.rom_path
                )
            except Exception:
                self.warning_label.set_text("Uh-Oh...")
                self.pb_label.set_text(
                    f"Error Swapping Models/Animations/Properties...\n{self._mumbo_error_message}"
                )
                raise
        if (
            (self.master.before_blue_egg_carry_value.get() != 100)
            or (self.master.after_blue_egg_carry_value.get() != 200)
            or (self.master.before_red_feather_carry_value.get() != 50)
            or (self.master.after_red_feather_carry_value.get() != 100)
            or (self.master.before_gold_feather_carry_value.get() != 10)
            or (self.master.after_gold_feather_carry_value.get() != 20)
        ):
            try:
                ...
                self.warning_label.set_text("Uh-Oh...")
                self.pb_label.set_text(
                    f"Error Adjusting Carrying Capacity...\n{self._mumbo_error_message}"
                )
                raise
        try:
    ...
    def _clean_up(self) -> None:
        ...
        if self.master.remove_files_var.get() == 1:
            clean_up = CleanUp(self.master.paths)
            clean_up._remove_bin_files()
            del clean_up
        gc.collect()
        self.master.logger.info("Cleaning Up Complete")

    def _randomization_process(self) -> None:
        """
        Runs through the setup, decompression, world manipulation, misc settings,
        compression, crc tool, and clean up functions in that order.
        ...
        try:
            ...
            self.warning_label.set_text("Uh-Oh...")
            self.pb_label.set_text(
                f"Error Correcting Checksums...\n{self._mumbo_error_message}"
            )
            raise
        self.progress_bar.update_bar(65)
        try:
            self.pb_label.set_text(
                "Hope This Works... *Eekum Bookum Eekum Bookum*\n(May Take A Minute Or Two)"
            )
            self._compress_main()
        except Exception:
            self.warning_label.set_text("Uh-Oh...")
            self.pb_label.set_text(
                f"Error Compressing Files...\n{self._mumbo_error_message}"
            )
            raise
        ...
        try:
            ...
            self.warning_label.set_text("Uh-Oh...")
            self.pb_label.set_text(
                f"Error Recalculating CRC...\n{self._mumbo_error_message}"
            )
            raise
        self.progress_bar.update_bar(95)
        try:
            self.pb_label.set_text(
                "Changed Mind! T-Rex Spell Too Good For This Playthrough!"
            )
            self._clean_up()
        except Exception:
            self.warning_label.set_text("Uh-Oh...")
    ...
    def _main(self) -> None:
        ...
        ########################
        # Label
        self.warning_label = self.App_Variable_Label(
            window=self.progress_bar_window,
            label_text="Bear And Bird Cannot Close Window.\nMust Complete ROM Transformation.\nHope This Works...",
        )
        self.warning_label.pack_label()
        # Mumbo Jumbo Talking
        self.frame_count = 10
        mumbo_jumbo_speaking_path = (
            Path(self.master.cwd) / "Pictures" / "Mumbo_Jumbo_Speaking.gif"
        )
        self.frames = [
            tk.PhotoImage(
                master=self.progress_bar_window,
                file=mumbo_jumbo_speaking_path,
                format="gif -index %i" % (i),
            )
            for i in range(self.frame_count)
        ]
        self.mumbo_talking_label = tk.Label(
            self.progress_bar_window, background="#F3E5AB"
        )
        self.mumbo_talking_label.pack()
        # Variable Label
        self.pb_label = self.App_Variable_Label(
            window=self.progress_bar_window, label_text="Progress Bar"
        )
        ...
        ##########################
        ### Close Window ##
        self.progress_bar_window.protocol(
            "WM_DELETE_WINDOW", self.progress_bar_window.destroy
        )
        ### Update Window ###
